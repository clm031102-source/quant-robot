# Round666 Next Steps Checklist

Use this after pulling latest `main`. Round666 is expected to be merged back to `main`; do not pull, merge, or revive old Round663, Round664, Round665, or Round666 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 882 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 53 offset 10 limit 5.
...
- Stop if not on a dedicated Round667 branch before provider work.
...
- Stop if fresh preview for shard 53 offset 10 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `600643.SH`, `002853.SZ`, `002032.SZ`, `002176.SZ`, and `300185.SZ`.
...
git switch -c codex/data-pipeline-financial-timeliness-round667-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round667-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round667-20260708
```
...
- Round666 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 877 / 1,000 unique symbols using `--financial-root data\processed`.
- Shard 53 offset 5 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `002872.SZ`, `300053.SZ`, `600128.SH`, `600007.SH`, `002585.SZ`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 217 processed rows, 14 empty requests, and 0 duplicate rows in the quality report.
- Aggregate source audit improved coverage to 882 unique symbols and 186,941 rows.
...
- Do not generate financial reporting timeliness factors from the 882-symbol cache.
