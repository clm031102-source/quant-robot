# Round635 Next Steps Checklist

Use this after pulling latest `main`. Round635 is expected to be merged back to `main`; do not pull, merge, or revive old Round633, Round634, or Round635 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 727 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 45 offset 15 limit 5.
...
- Stop if not on a dedicated Round636 branch before provider work.
...
- Stop if fresh preview for shard 45 offset 15 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `601872.SH`, `301039.SZ`, `603377.SH`, `000901.SZ`, and `600018.SH`.
...
git switch -c codex/data-pipeline-financial-timeliness-round636-20260707
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round636-20260707
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round636-20260707
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 45 --symbol-offset 15 --symbol-limit 5 --output-dir data\reports\round636_financial_statement_overlap_preview_shard45_offset15_limit5_financial_roots_20260707
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 45 --symbol-offset 15 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round636_financial_statement_shard45_offset15_limit5_20260707
```
...
- Round635 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 722 / 1,000 unique symbols.
- Shard 45 offset 10 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `300576.SZ`, `001325.SZ`, `600868.SH`, `600283.SH`, `002596.SZ`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 180 processed rows, and 0 duplicate rows in the quality report.
- Aggregate source audit improved coverage to 727 unique symbols and 153,965 rows.
...
- Do not generate financial reporting timeliness factors from the 727-symbol cache.
