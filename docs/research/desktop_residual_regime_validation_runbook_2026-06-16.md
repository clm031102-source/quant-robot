...
```powershell
python scripts\run_market_regime_coverage.py --regime-curve-glob "data/reports/walk_forward_tushare_moneyflow_residual_regime/fold_*/test/*/regime_curve.csv" --walk-forward-folds data/reports/walk_forward_tushare_moneyflow_residual_regime/walk_forward_folds.csv --output-dir data/reports/market_regime_coverage_tushare_moneyflow_residual_regime --min-regimes 2 --min-rows-per-regime 5 --min-allowed-rows 5 --min-blocked-rows 5 --require-sufficient
```
