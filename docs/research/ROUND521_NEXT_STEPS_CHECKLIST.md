# Round521 Next Steps Checklist

Use this page before continuing from `codex/factor-batch-cn-stock-profit-mining-20260704`.

## Current State

- Current loop round: 18 after the Round504 review-agent baseline.
...
- Latest quota tooling: analyst-report quota packs now include explicit `provenance.machine`, `provenance.task`, and `provenance.branch`.
- Latest provenance-aware pack export: `data\reports\round521_analyst_quota_pack_provenance_20260705`, with `exported_report_count=8`, machine `office_desktop`, task `factor_batch`, and branch `codex/factor-batch-cn-stock-profit-mining-20260704`.
...
- Latest full profile: `scripts/run_checks.py --profile laptop-integration --execute` passed with 95 unit tests, Python compile, project audit, and laptop project-sync audit.
...
The manifest and terminal JSON should show:
...
```powershell
.\.venv\Scripts\python.exe scripts\run_tushare_analyst_report_cache.py --start-date 2024-04-01 --end-date 2024-04-30 --output-dir data\reports\round522_analyst_report_revision_cache_202404_<YYYYMMDD> --processed-output-dir data\processed\round522_analyst_report_revision_cache_202404_<YYYYMMDD> --window-frequency MS --request-sleep-seconds 0 --max-rows-per-window 5000 --quota-report-root data\reports --quota-report-root <imported_pack_root_from_other_machine> --quota-output-dir data\reports\round522_cache_cli_quota_preflight_<YYYYMMDD> --quota-preflight-only
```
...
2. Export or gather provenance-aware quota packs from every active workstation.
...
```powershell
.\.venv\Scripts\python.exe scripts\run_analyst_report_revision_prescreen.py --report-root data\processed\round463_analyst_report_revision_source_smoke_20260704 --report-root data\processed\round504_analyst_report_revision_cache_202402_20260705 --report-root data\processed\round505_analyst_report_revision_cache_202403_20260705 --report-root data\processed\round522_analyst_report_revision_cache_202404_<YYYYMMDD> --stock-basic data\processed\cn_stock_metadata --output-dir data\reports\round522_analyst_report_revision_prescreen_202401_202404_<YYYYMMDD> --analysis-start-date 2024-01-01 --analysis-end-date 2024-06-30 --horizons 5,20 --execution-lag 1 --pit-lag-trade-days 1 --min-cross-section 20 --min-ic-observations 1 --min-industries 2 --min-assets-per-industry 2 --min-signal-date-amount 10000000
```
...
- Terminal output and JSON evidence include `target_date_matches_generated_at=true`, `quota_scope="local_report_roots_only"`, `duplicate_evidence_rows`, `duplicate_window_rows`, all scanned `report_roots`, and pack provenance in the pack manifests.
