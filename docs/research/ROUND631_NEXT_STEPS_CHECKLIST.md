# Round631 Next Steps Checklist

Use this after pulling latest `main`. Round631 is expected to be merged back to `main`; do not pull, merge, or revive old Round629, Round630, or Round631 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 707 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 44 offset 15 limit 5.
...
- Stop if not on a dedicated Round632 branch before provider work.
...
- Stop if fresh preview for shard 44 offset 15 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `002149.SZ`, `002526.SZ`, `002229.SZ`, `002051.SZ`, and `300133.SZ`.
...
git switch -c codex/data-pipeline-financial-timeliness-round632-20260707
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round632-20260707
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round632-20260707
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 44 --symbol-offset 15 --symbol-limit 5 --output-dir data\reports\round632_financial_statement_overlap_preview_shard44_offset15_limit5_financial_roots_20260707
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 44 --symbol-offset 15 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round632_financial_statement_shard44_offset15_limit5_20260707
```
...
- Round631 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 702 / 1,000 unique symbols.
- Shard 44 offset 10 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `600064.SH`, `002457.SZ`, `002961.SZ`, `002790.SZ`, `002005.SZ`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 230 processed rows, and 20 duplicate rows in the quality report.
- Aggregate source audit improved coverage to 707 unique symbols and 149,979 rows.
...
- Do not generate financial reporting timeliness factors from the 707-symbol cache.
