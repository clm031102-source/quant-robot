# Round676 Next Steps Checklist

Use this after pulling latest `main`. Round676 is expected to be merged back to `main`; do not pull, merge, or revive old Round673, Round674, Round675, or Round676 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 932 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 56 offset 0 limit 5.
...
- Stop if not on a dedicated Round677 branch before provider work.
...
- Stop if fresh preview for shard 56 offset 0 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `002161.SZ`, `300883.SZ`, `002843.SZ`, `600961.SH`, and `601899.SH`.
...
git switch -c codex/data-pipeline-financial-timeliness-round677-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round677-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round677-20260708
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 56 --symbol-offset 0 --symbol-limit 5 --output-dir data\reports\round677_financial_statement_overlap_preview_shard56_offset0_limit5_financial_roots_20260708
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 56 --symbol-offset 0 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round677_financial_statement_shard56_offset0_limit5_20260708
```
...
- Round676 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 927 / 1,000 unique symbols using `--financial-root data\processed`.
- Shard 55 offset 15 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `002830.SZ`, `002736.SZ`, `600377.SH`, `002178.SZ`, `002972.SZ`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 216 processed rows, 22 empty requests, and 1 duplicate row in the passing quality report.
- Aggregate source audit improved coverage to 932 unique symbols and 197,543 rows.
...
- Do not generate financial reporting timeliness factors from the 932-symbol cache.
