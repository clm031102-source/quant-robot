# Round520 Next Steps Checklist

Use this page before continuing from `codex/factor-batch-cn-stock-profit-mining-20260704`.

## Current State

- Current loop round: 17 after the Round504 review-agent baseline.
...
- Latest quota tooling: analyst-report quota preflight now records duplicate pack evidence details in `duplicate_window_rows`.
- Latest duplicate-detail preflight: `data\reports` plus the same Round519 pack twice still counted only 2 same-day provider request windows, skipped 2 duplicate evidence rows, wrote duplicate row details, blocked with `daily_provider_request_budget_exhausted`, and exited `3`.
...
Round520 makes skipped duplicate evidence auditable:

- `summary.duplicate_evidence_rows` records the number skipped.
- `duplicate_window_rows` records kept and duplicate report paths plus window/status/fingerprint details.
- The Markdown report includes a `Duplicate Evidence Rows` table.
...
```powershell
.\.venv\Scripts\python.exe scripts\run_tushare_analyst_report_cache.py --start-date 2024-04-01 --end-date 2024-04-30 --output-dir data\reports\round521_analyst_report_revision_cache_202404_<YYYYMMDD> --processed-output-dir data\processed\round521_analyst_report_revision_cache_202404_<YYYYMMDD> --window-frequency MS --request-sleep-seconds 0 --max-rows-per-window 5000 --quota-report-root data\reports --quota-report-root <imported_pack_root_from_other_machine> --quota-output-dir data\reports\round521_cache_cli_quota_preflight_<YYYYMMDD> --quota-preflight-only
```
...
- Terminal output and JSON evidence include `target_date_matches_generated_at=true`, `quota_scope="local_report_roots_only"`, `duplicate_evidence_rows`, `duplicate_window_rows`, and all scanned `report_roots`.
