# Round647 Next Steps Checklist

Use this after pulling latest `main`. Round647 is expected to be merged back to `main`; do not pull, merge, or revive old Round645, Round646, or Round647 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 787 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 48 offset 15 limit 5.
...
- Stop if not on a dedicated Round648 branch before provider work.
...
- Stop if fresh preview for shard 48 offset 15 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `002254.SZ`, `000863.SZ`, `002219.SZ`, `002788.SZ`, and `300046.SZ`.
...
git switch -c codex/data-pipeline-financial-timeliness-round648-20260707
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round648-20260707
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round648-20260707
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 48 --symbol-offset 15 --symbol-limit 5 --output-dir data\reports\round648_financial_statement_overlap_preview_shard48_offset15_limit5_financial_roots_20260707
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 48 --symbol-offset 15 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round648_financial_statement_shard48_offset15_limit5_20260707
```
...
- Round647 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 782 / 1,000 unique symbols.
- Shard 48 offset 10 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `002004.SZ`, `301231.SZ`, `000915.SZ`, `000830.SZ`, `600579.SH`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 202 processed rows, and 0 duplicate rows in the quality report.
- Empty requests were 56 and should remain visible as a source-QA watch item.
- Aggregate source audit improved coverage to 787 unique symbols and 166,684 rows.
...
- Do not generate financial reporting timeliness factors from the 787-symbol cache.
