# Round515 Next Steps Checklist

Use this page before continuing from `codex/factor-batch-cn-stock-profit-mining-20260704`.

## Current State

- Current loop round: 12 after the Round504 review-agent baseline.
...
- Latest tooling hardening: provider-backed analyst-report cache now fails closed if `--quota-target-date` differs from the local generated date. Nonlocal target dates remain valid only for audit evidence or `--quota-preflight-only`.
- Latest focused tests: `tests/unit/test_analyst_report_quota_preflight.py` passed with 15 tests.
- Latest full profile: `scripts/run_checks.py --profile laptop-integration --execute` passed with 88 tests, Python compile, project audit, and laptop project-sync audit.
...
```powershell
.\.venv\Scripts\python.exe scripts\run_tushare_analyst_report_cache.py --start-date 2024-04-01 --end-date 2024-04-30 --output-dir data\reports\round516_analyst_report_revision_cache_202404_<YYYYMMDD> --processed-output-dir data\processed\round516_analyst_report_revision_cache_202404_<YYYYMMDD> --window-frequency MS --request-sleep-seconds 0 --max-rows-per-window 5000 --quota-output-dir data\reports\round516_cache_cli_quota_preflight_<YYYYMMDD> --quota-preflight-only
```
