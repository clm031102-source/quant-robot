# Round672 Next Steps Checklist

Use this after pulling latest `main`. Round672 is expected to be merged back to `main`; do not pull, merge, or revive old Round669, Round670, Round671, or Round672 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 912 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 55 offset 0 limit 5.
...
- Stop if not on a dedicated Round673 branch before provider work.
...
- Stop if fresh preview for shard 55 offset 0 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `600176.SH`, `300149.SZ`, `300112.SZ`, `002028.SZ`, and `600779.SH`.
...
git switch -c codex/data-pipeline-financial-timeliness-round673-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round673-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round673-20260708
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 55 --symbol-offset 0 --symbol-limit 5 --output-dir data\reports\round673_financial_statement_overlap_preview_shard55_offset0_limit5_financial_roots_20260708
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 55 --symbol-offset 0 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round673_financial_statement_shard55_offset0_limit5_20260708
```
...
- Round672 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 907 / 1,000 unique symbols using `--financial-root data\processed`.
- Shard 54 offset 15 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `000980.SZ`, `601000.SH`, `000966.SZ`, `600508.SH`, `002034.SZ`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 220 processed rows, 3 empty requests, and 0 duplicate rows in the quality report.
- Aggregate source audit improved coverage to 912 unique symbols and 193,431 rows.
...
- Do not generate financial reporting timeliness factors from the 912-symbol cache.
