# Round668 Next Steps Checklist

Use this after pulling latest `main`. Round668 is expected to be merged back to `main`; do not pull, merge, or revive old Round665, Round666, Round667, or Round668 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 892 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 54 offset 0 limit 5.
...
- Stop if not on a dedicated Round669 branch before provider work.
...
- Stop if fresh preview for shard 54 offset 0 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `002678.SZ`, `002256.SZ`, `600054.SH`, `600223.SH`, and `600808.SH`.
...
git switch -c codex/data-pipeline-financial-timeliness-round669-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round669-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round669-20260708
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 54 --symbol-offset 0 --symbol-limit 5 --output-dir data\reports\round669_financial_statement_overlap_preview_shard54_offset0_limit5_financial_roots_20260708
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 54 --symbol-offset 0 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round669_financial_statement_shard54_offset0_limit5_20260708
```
...
- Round668 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 887 / 1,000 unique symbols using `--financial-root data\processed`.
- Shard 53 offset 15 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `002291.SZ`, `002061.SZ`, `300251.SZ`, `600136.SH`, `603529.SH`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 206 processed rows, 45 empty requests, and 0 duplicate rows in the quality report.
- Aggregate source audit improved coverage to 892 unique symbols and 189,064 rows.
...
- Do not generate financial reporting timeliness factors from the 892-symbol cache.
