# Round649 Next Steps Checklist

Use this after pulling latest `main`. Round649 is expected to be merged back to `main`; do not pull, merge, or revive old Round647, Round648, or Round649 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 797 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 49 offset 5 limit 5.
...
- Stop if not on a dedicated Round650 branch before provider work.
...
- Stop if fresh preview for shard 49 offset 5 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `002011.SZ`, `002167.SZ`, `002535.SZ`, `002243.SZ`, and `002060.SZ`.
...
git switch -c codex/data-pipeline-financial-timeliness-round650-20260707
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round650-20260707
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round650-20260707
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 49 --symbol-offset 5 --symbol-limit 5 --output-dir data\reports\round650_financial_statement_overlap_preview_shard49_offset5_limit5_financial_roots_20260707
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 49 --symbol-offset 5 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round650_financial_statement_shard49_offset5_limit5_20260707
```
...
- Round649 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 792 / 1,000 unique symbols.
- Shard 49 offset 0 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `600058.SH`, `600082.SH`, `002522.SZ`, `001236.SZ`, `002798.SZ`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 235 processed rows, and 28 duplicate rows in the quality report.
- Empty requests were 51 and should remain visible as a source-QA watch item.
- Aggregate source audit improved coverage to 797 unique symbols and 168,872 rows.
...
- Do not generate financial reporting timeliness factors from the 797-symbol cache.
