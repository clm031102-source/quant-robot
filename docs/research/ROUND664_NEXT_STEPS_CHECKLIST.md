# Round664 Next Steps Checklist

Use this after pulling latest `main`. Round664 is expected to be merged back to `main`; do not pull, merge, or revive old Round661, Round662, Round663, or Round664 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 872 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 53 offset 0 limit 5.
...
- Stop if not on a dedicated Round665 branch before provider work.
...
- Stop if fresh preview for shard 53 offset 0 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `000881.SZ`, `601798.SH`, `002427.SZ`, `000886.SZ`, and `002223.SZ`.
...
git switch -c codex/data-pipeline-financial-timeliness-round665-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round665-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round665-20260708
```
...
- Round664 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 867 / 1,000 unique symbols using `--financial-root data\processed`.
- Shard 52 offset 15 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `002321.SZ`, `603029.SH`, `002170.SZ`, `600825.SH`, `000919.SZ`.
...
- Aggregate source audit improved coverage to 872 unique symbols and 184,732 rows.
...
- Do not generate financial reporting timeliness factors from the 872-symbol cache.
