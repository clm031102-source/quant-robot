# Round653 Next Steps Checklist

Use this after pulling latest `main`. Round653 is expected to be merged back to `main`; do not pull, merge, or revive old Round651, Round652, or Round653 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 817 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 50 offset 5 limit 5.
...
- Stop if not on a dedicated Round654 branch before provider work.
...
- Stop if fresh preview for shard 50 offset 5 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `002671.SZ`, `601866.SH`, `600686.SH`, `000903.SZ`, and `601008.SH`.
...
git switch -c codex/data-pipeline-financial-timeliness-round654-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round654-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round654-20260708
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 50 --symbol-offset 5 --symbol-limit 5 --output-dir data\reports\round654_financial_statement_overlap_preview_shard50_offset5_limit5_financial_roots_20260708
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 50 --symbol-offset 5 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round654_financial_statement_shard50_offset5_limit5_20260708
```
...
- Round653 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 812 / 1,000 unique symbols.
- Shard 50 offset 0 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `002342.SZ`, `300665.SZ`, `301595.SZ`, `600886.SH`, `600461.SH`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 187 processed rows, 118 empty requests, and 1 duplicate row in the quality report.
- Aggregate source audit improved coverage to 817 unique symbols and 172,815 rows.
...
- Do not generate financial reporting timeliness factors from the 817-symbol cache.
