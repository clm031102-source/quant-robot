# Round637 Next Steps Checklist

Use this after pulling latest `main`. Round637 is expected to be merged back to `main`; do not pull, merge, or revive old Round635, Round636, or Round637 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 737 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 46 offset 5 limit 5.
...
- Stop if not on a dedicated Round638 branch before provider work.
...
- Stop if fresh preview for shard 46 offset 5 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `300122.SZ`, `300066.SZ`, `000821.SZ`, `600809.SH`, and `002277.SZ`.
...
git switch -c codex/data-pipeline-financial-timeliness-round638-20260707
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round638-20260707
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round638-20260707
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 46 --symbol-offset 5 --symbol-limit 5 --output-dir data\reports\round638_financial_statement_overlap_preview_shard46_offset5_limit5_financial_roots_20260707
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 46 --symbol-offset 5 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round638_financial_statement_shard46_offset5_limit5_20260707
```
...
- Round637 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 732 / 1,000 unique symbols.
- Shard 46 offset 0 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `000899.SZ`, `600188.SH`, `603995.SH`, `000890.SZ`, `600819.SH`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 210 processed rows, and 0 duplicate rows in the quality report.
- Aggregate source audit improved coverage to 737 unique symbols and 156,060 rows.
...
- Do not generate financial reporting timeliness factors from the 737-symbol cache.
