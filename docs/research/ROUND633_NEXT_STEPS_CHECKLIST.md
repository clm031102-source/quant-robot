# Round633 Next Steps Checklist

Use this after pulling latest `main`. Round633 is expected to be merged back to `main`; do not pull, merge, or revive old Round631, Round632, or Round633 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 717 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 45 offset 5 limit 5.
...
- Stop if not on a dedicated Round634 branch before provider work.
...
- Stop if fresh preview for shard 45 offset 5 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `001328.SZ`, `002110.SZ`, `002345.SZ`, `002685.SZ`, and `002282.SZ`.
...
git switch -c codex/data-pipeline-financial-timeliness-round634-20260707
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round634-20260707
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round634-20260707
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 45 --symbol-offset 5 --symbol-limit 5 --output-dir data\reports\round634_financial_statement_overlap_preview_shard45_offset5_limit5_financial_roots_20260707
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 45 --symbol-offset 5 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round634_financial_statement_shard45_offset5_limit5_20260707
```
...
- Round633 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 712 / 1,000 unique symbols.
- Shard 45 offset 0 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `300947.SZ`, `603787.SH`, `300192.SZ`, `002053.SZ`, `600706.SH`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 204 processed rows, and 1 duplicate row in the quality report.
- Aggregate source audit improved coverage to 717 unique symbols and 152,092 rows.
...
- Do not generate financial reporting timeliness factors from the 717-symbol cache.
