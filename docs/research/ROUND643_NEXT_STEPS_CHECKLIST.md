# Round643 Next Steps Checklist

Use this after pulling latest `main`. Round643 is expected to be merged back to `main`; do not pull, merge, or revive old Round641, Round642, or Round643 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 767 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 47 offset 15 limit 5.
...
- Stop if not on a dedicated Round644 branch before provider work.
...
- Stop if fresh preview for shard 47 offset 15 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `002891.SZ`, `600988.SH`, `002351.SZ`, `002255.SZ`, and `002107.SZ`.
...
git switch -c codex/data-pipeline-financial-timeliness-round644-20260707
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round644-20260707
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round644-20260707
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 47 --symbol-offset 15 --symbol-limit 5 --output-dir data\reports\round644_financial_statement_overlap_preview_shard47_offset15_limit5_financial_roots_20260707
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 47 --symbol-offset 15 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round644_financial_statement_shard47_offset15_limit5_20260707
```
...
- Round643 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 762 / 1,000 unique symbols.
- Shard 47 offset 10 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `600531.SH`, `600490.SH`, `002540.SZ`, `001227.SZ`, `002216.SZ`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 220 processed rows, and 0 duplicate rows in the quality report.
- Empty requests were 22 and should remain visible as a source-QA watch item.
- Aggregate source audit improved coverage to 767 unique symbols and 162,490 rows.
...
- Do not generate financial reporting timeliness factors from the 767-symbol cache.
