# Round667 Next Steps Checklist

Use this after pulling latest `main`. Round667 is expected to be merged back to `main`; do not pull, merge, or revive old Round664, Round665, Round666, or Round667 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 887 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 53 offset 15 limit 5.
...
- Stop if not on a dedicated Round668 branch before provider work.
...
- Stop if fresh preview for shard 53 offset 15 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `002291.SZ`, `002061.SZ`, `300251.SZ`, `600136.SH`, and `603529.SH`.
...
git switch -c codex/data-pipeline-financial-timeliness-round668-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round668-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round668-20260708
```
...
- Round667 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 882 / 1,000 unique symbols using `--financial-root data\processed`.
- Shard 53 offset 10 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `600643.SH`, `002853.SZ`, `002032.SZ`, `002176.SZ`, `300185.SZ`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 217 processed rows, 11 empty requests, and 0 duplicate rows in the quality report.
- Aggregate source audit improved coverage to 887 unique symbols and 188,037 rows.
...
- Do not generate financial reporting timeliness factors from the 887-symbol cache.
