# Round632 Next Steps Checklist

Use this after pulling latest `main`. Round632 is expected to be merged back to `main`; do not pull, merge, or revive old Round630, Round631, or Round632 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 712 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 45 offset 0 limit 5.
...
- Stop if not on a dedicated Round633 branch before provider work.
...
- Stop if fresh preview for shard 45 offset 0 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `300947.SZ`, `603787.SH`, `300192.SZ`, `002053.SZ`, and `600706.SH`.
...
git switch -c codex/data-pipeline-financial-timeliness-round633-20260707
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round633-20260707
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round633-20260707
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 45 --symbol-offset 0 --symbol-limit 5 --output-dir data\reports\round633_financial_statement_overlap_preview_shard45_offset0_limit5_financial_roots_20260707
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 45 --symbol-offset 0 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round633_financial_statement_shard45_offset0_limit5_20260707
```
...
- Round632 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 707 / 1,000 unique symbols.
- Shard 44 offset 15 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `002149.SZ`, `002526.SZ`, `002229.SZ`, `002051.SZ`, `300133.SZ`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 220 processed rows, and 0 duplicate rows in the quality report.
- Aggregate source audit improved coverage to 712 unique symbols and 151,070 rows.
...
- Do not generate financial reporting timeliness factors from the 712-symbol cache.
