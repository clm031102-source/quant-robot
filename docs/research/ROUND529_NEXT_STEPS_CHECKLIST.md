# Round529 Next Steps Checklist

Use this page before continuing from `codex/factor-batch-cn-stock-profit-mining-20260704`.

## Current State

- Current loop round: 26 after the Round504 review-agent baseline.
...
- if analyst cache remains blocked, write a source-repair plan for LPR coverage or optimize the long-window external-feed join smoke;
- prepare a future HK-hold candidate-plan template only if it is a genuinely new mechanism and not a rerun of old northbound accumulation or crowding/reversal.
...
```powershell
.\.venv\Scripts\python.exe scripts\run_tushare_analyst_report_cache.py --start-date 2024-04-01 --end-date 2024-04-30 --output-dir data\reports\round530_analyst_report_revision_cache_202404_<YYYYMMDD> --processed-output-dir data\processed\round530_analyst_report_revision_cache_202404_<YYYYMMDD> --window-frequency MS --request-sleep-seconds 0 --max-rows-per-window 5000 --quota-report-root data\reports --quota-report-root data\reports\round521_analyst_quota_pack_provenance_20260705 --quota-output-dir data\reports\round530_required_machine_quota_preflight_<YYYYMMDD> --quota-required-pack-machine office_desktop --quota-required-pack-machine highspec_desktop --quota-required-pack-machine laptop --quota-pack-machine-note "highspec_desktop=<reason and timestamp if still unavailable>" --quota-pack-machine-note "laptop=<reason and timestamp if still unavailable>" --quota-preflight-only
```
...
- Margin-credit remains hibernated because Round193 residual and dedup review failed.
- LPR/macro-rate factors remain blocked until LPR non-missing coverage is repaired.
- SHIBOR may be reviewed only as a regime-control input after long-cycle validation, not as a standalone stock rank.

Do not run external-feed IC, portfolio, promotion, or final-holdout steps from the Round528 source audit.

Allowed external-feed work while analyst cache is blocked:

- write an LPR coverage repair or backfill feasibility plan;
- optimize or profile the long-window external-feed join smoke;
