# Round679 Next Steps Checklist

Use this after pulling latest `main`. Round679 is expected to be merged back to `main`; do not pull, merge, or revive old Round675, Round676, Round677, Round678, or Round679 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 947 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 56 offset 15 limit 5.
...
- Stop if not on a dedicated Round680 branch before provider work.
...
- Stop if fresh preview for shard 56 offset 15 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `002893.SZ`, `000823.SZ`, `002133.SZ`, `605136.SH`, and `002392.SZ`.
...
git switch -c codex/data-pipeline-financial-timeliness-round680-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round680-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round680-20260708
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 56 --symbol-offset 15 --symbol-limit 5 --output-dir data\reports\round680_financial_statement_overlap_preview_shard56_offset15_limit5_financial_roots_20260708
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 56 --symbol-offset 15 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round680_financial_statement_shard56_offset15_limit5_20260708
```
...
- Round679 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 942 / 1,000 unique symbols using `--financial-root data\processed`.
- Shard 56 offset 10 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `300024.SZ`, `002198.SZ`, `600882.SH`, `300002.SZ`, `300350.SZ`.
...
- Aggregate source audit improved coverage to 947 unique symbols and 200,755 rows.
...
- Do not generate financial reporting timeliness factors from the 947-symbol cache.
