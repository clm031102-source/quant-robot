# Round688 Next Steps Checklist

Use this after pulling latest `main`. Round688 is expected to be merged back to `main`; do not pull, merge, or revive old Round675, Round676, Round677, Round678, Round679, Round680, Round681, Round682, Round683, Round684, Round685, Round686, Round687, or Round688 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 992 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 59 offset 0 limit 5.
...
- Stop if not on a dedicated Round689 branch before provider work.
...
- Stop if fresh preview for shard 59 offset 0 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `601018.SH`, `000875.SZ`, `600348.SH`, `002210.SZ`, and `600293.SH`.
...
git switch -c codex/data-pipeline-financial-timeliness-round689-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round689-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round689-20260708
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 59 --symbol-offset 0 --symbol-limit 5 --output-dir data\reports\round689_financial_statement_overlap_preview_shard59_offset0_limit5_financial_roots_20260708
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 59 --symbol-offset 0 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round689_financial_statement_shard59_offset0_limit5_20260708
```
...
- Round688 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 987 / 1,000 unique symbols using `--financial-root data\processed`.
- Shard 58 offset 15 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `601199.SH`, `003037.SZ`, `603167.SH`, `600066.SH`, `000981.SZ`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 206 processed rows, 44 empty requests, and 0 duplicate rows in the quality report.
- Aggregate source audit improved coverage to 992 unique symbols and 210,178 rows.
...
- Do not generate financial reporting timeliness factors from the 992-symbol cache.
