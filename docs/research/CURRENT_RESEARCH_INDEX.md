...
ETFoutcome join. Fifty related synthetic tests pass. The retained separate Fraction/calendar checker
reconciles 44 full/16 later intervals and expands 2,672 daily states; it is same-researcher evidence.
Independent integration matches all 84 input roles, 11 runtime pins, seven execution protocols and
the registration/claim/result/closure chain without rerunning real states or ETF outcomes. No financial
...
entries,270annual contributions and144position episodes reconcile from the recorded-fill ledger;5000scalar block
replicates reproduce the interval. This is not full execution-causality or missing-order verification.
Integration separately confirms all27scenario/asset/config identities, primary mapping and all seven
screen predicates;55related integration tests pass.62related tests were reported before execution.24source requests remain
...
is[-66.36,+130.91]CNY. Its lower bound fails: the hypothesis is closed for insufficient evidence,
...
- Source audit: [Tushare NAV source readiness](cn-etf-tushare-nav-source-readiness-20260729.md); 705,081 point-in-time rows across 1,067 assets.
- Governing closeout: [delayed-NAV prescreen invalidation](cn-etf-delayed-nav-premium-prescreen-20260729.md).
...
- PCF vendor request: `configs/cn_etf_pcf_vendor_request_20260728.json`; delivery contract: `configs/cn_etf_pcf_delivery_contract.json`; survivorship-reviewed target builder: `scripts/build_cn_etf_pcf_target_universe.py`; structural validator: `scripts/validate_cn_etf_pcf_delivery.py`; cross-exchange source-readiness gate: `scripts/run_cn_etf_pcf_source_readiness.py`
