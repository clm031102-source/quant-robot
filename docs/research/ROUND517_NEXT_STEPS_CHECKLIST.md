# Round517 Next Steps Checklist

Use this page before continuing from `codex/factor-batch-cn-stock-profit-mining-20260704`.

## Current State

- Current loop round: 14 after the Round504 review-agent baseline.
...
- Latest tooling hardening: `--skip-quota-preflight` attempts now write durable `skip_quota_preflight_audit.json` and `.md` evidence whether allowed or blocked.
...
- Latest skip guard evidence: skip with an empty processed-output directory is blocked with `skip_quota_preflight_requires_cached_processed_windows` and writes durable skip-audit files.
...
- It writes `skip_quota_preflight_audit.json` and `skip_quota_preflight_audit.md` under the cache output directory.
...
```powershell
.\.venv\Scripts\python.exe scripts\run_tushare_analyst_report_cache.py --start-date 2024-04-01 --end-date 2024-04-30 --output-dir data\reports\round518_analyst_report_revision_cache_202404_<YYYYMMDD> --processed-output-dir data\processed\round518_analyst_report_revision_cache_202404_<YYYYMMDD> --window-frequency MS --request-sleep-seconds 0 --max-rows-per-window 5000 --quota-output-dir data\reports\round518_cache_cli_quota_preflight_<YYYYMMDD> --quota-preflight-only
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_analyst_report_revision_prescreen.py --report-root data\processed\round463_analyst_report_revision_source_smoke_20260704 --report-root data\processed\round504_analyst_report_revision_cache_202402_20260705 --report-root data\processed\round505_analyst_report_revision_cache_202403_20260705 --report-root data\processed\round518_analyst_report_revision_cache_202404_<YYYYMMDD> --stock-basic data\processed\cn_stock_metadata --output-dir data\reports\round518_analyst_report_revision_prescreen_202401_202404_<YYYYMMDD> --analysis-start-date 2024-01-01 --analysis-end-date 2024-06-30 --horizons 5,20 --execution-lag 1 --pit-lag-trade-days 1 --min-cross-section 20 --min-ic-observations 1 --min-industries 2 --min-assets-per-industry 2 --min-signal-date-amount 10000000
```
