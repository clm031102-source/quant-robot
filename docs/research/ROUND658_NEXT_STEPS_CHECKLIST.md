# Round658 Next Steps Checklist

Use this after pulling latest `main`. Round658 is expected to be merged back to `main`; do not pull, merge, or revive old Round656, Round657, or Round658 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 842 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 51 offset 10 limit 5.
...
- Stop if not on a dedicated Round659 branch before provider work.
...
- Stop if fresh preview for shard 51 offset 10 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `002673.SZ`, `600033.SH`, `002153.SZ`, `300669.SZ`, and `002151.SZ`.
...
git switch -c codex/data-pipeline-financial-timeliness-round659-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round659-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round659-20260708
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 51 --symbol-offset 10 --symbol-limit 5 --output-dir data\reports\round659_financial_statement_overlap_preview_shard51_offset10_limit5_financial_roots_20260708
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 51 --symbol-offset 10 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round659_financial_statement_shard51_offset10_limit5_20260708
```
...
- Round658 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 837 / 1,000 unique symbols.
- Shard 51 offset 5 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `603337.SH`, `600673.SH`, `002625.SZ`, `601890.SH`, `002822.SZ`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 216 processed rows, 16 empty requests, and 0 duplicate rows in the quality report.
- Aggregate source audit improved coverage to 842 unique symbols and 178,286 rows.
...
- Do not generate financial reporting timeliness factors from the 842-symbol cache.
