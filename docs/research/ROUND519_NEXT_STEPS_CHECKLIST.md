# Round519 Next Steps Checklist

Use this page before continuing from `codex/factor-batch-cn-stock-profit-mining-20260704`.

## Current State

- Current loop round: 16 after the Round504 review-agent baseline.
...
- Latest quota tooling: analyst-report quota packs now carry stable source fingerprints, and preflight deduplicates repeated pack evidence.
- Latest real quota pack export: `data\reports\round519_analyst_quota_pack_dedup_20260705`, with `exported_report_count=8`.
- Latest pack dedup preflight: `data\reports` plus the same Round519 pack twice still counted only 2 same-day provider request windows, skipped 2 duplicate evidence rows, blocked with `daily_provider_request_budget_exhausted`, and exited `3`.
...
- Latest full profile: `scripts/run_checks.py --profile laptop-integration --execute` passed with 94 unit tests, Python compile, project audit, and laptop project-sync audit.
...
Round519 makes repeated pack evidence safe:

- Broad exporter scans skip old quota-pack internals.
- Pack exports include `quota_pack_source_fingerprint`.
- Preflight deduplicates rows with the same source fingerprint, date, window, status, and rate-limit fields.
- `duplicate_evidence_rows` records skipped duplicate evidence.
- Ordinary local cache reports from different paths remain distinct and still count separately.
...
```powershell
.\.venv\Scripts\python.exe scripts\run_tushare_analyst_report_cache.py --start-date 2024-04-01 --end-date 2024-04-30 --output-dir data\reports\round520_analyst_report_revision_cache_202404_<YYYYMMDD> --processed-output-dir data\processed\round520_analyst_report_revision_cache_202404_<YYYYMMDD> --window-frequency MS --request-sleep-seconds 0 --max-rows-per-window 5000 --quota-report-root data\reports --quota-report-root <imported_pack_root_from_other_machine> --quota-output-dir data\reports\round520_cache_cli_quota_preflight_<YYYYMMDD> --quota-preflight-only
```
...
- Round523 should run the next two-agent checkpoint if no earlier interruption changes the loop count.

## Success Signs

- Actual-date cache CLI preflight with local reports plus all available quota packs allows one request window and exits `0`.
- Terminal output and JSON evidence include `target_date_matches_generated_at=true`, `quota_scope="local_report_roots_only"`, `duplicate_evidence_rows`, and all scanned `report_roots`.
