# Round670 Next Steps Checklist

Use this after pulling latest `main`. Round670 is expected to be merged back to `main`; do not pull, merge, or revive old Round667, Round668, Round669, or Round670 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 902 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 54 offset 10 limit 5.
...
- Stop if not on a dedicated Round671 branch before provider work.
...
- Stop if fresh preview for shard 54 offset 10 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `600101.SH`, `601158.SH`, `002742.SZ`, `601919.SH`, and `600733.SH`.
...
git switch -c codex/data-pipeline-financial-timeliness-round671-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round671-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round671-20260708
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 54 --symbol-offset 10 --symbol-limit 5 --output-dir data\reports\round671_financial_statement_overlap_preview_shard54_offset10_limit5_financial_roots_20260708
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 54 --symbol-offset 10 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round671_financial_statement_shard54_offset10_limit5_20260708
```
...
- Round670 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 897 / 1,000 unique symbols using `--financial-root data\processed`.
- Shard 54 offset 5 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `002494.SZ`, `002903.SZ`, `002347.SZ`, `300758.SZ`, `601118.SH`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 210 processed rows, 42 empty requests, and 0 duplicate rows in the quality report.
- Aggregate source audit improved coverage to 902 unique symbols and 191,225 rows.
...
- Do not generate financial reporting timeliness factors from the 902-symbol cache.
