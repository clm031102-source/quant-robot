# Round686 Next Steps Checklist

Use this after pulling latest `main`. Round686 is expected to be merged back to `main`; do not pull, merge, or revive old Round675, Round676, Round677, Round678, Round679, Round680, Round681, Round682, Round683, Round684, Round685, or Round686 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 982 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 58 offset 10 limit 5.
...
- Stop if not on a dedicated Round687 branch before provider work.
...
- Stop if fresh preview for shard 58 offset 10 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `002943.SZ`, `002438.SZ`, `300798.SZ`, `603033.SH`, and `600116.SH`.
...
git switch -c codex/data-pipeline-financial-timeliness-round687-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round687-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round687-20260708
```
...
- Round686 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 977 / 1,000 unique symbols using `--financial-root data\processed`.
- Shard 58 offset 5 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `002310.SZ`, `600138.SH`, `600315.SH`, `600782.SH`, `002563.SZ`.
...
- Aggregate source audit improved coverage to 982 unique symbols and 208,100 rows.
...
- Do not generate financial reporting timeliness factors from the 982-symbol cache.
