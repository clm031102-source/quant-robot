# Round682 Next Steps Checklist

Use this after pulling latest `main`. Round682 is expected to be merged back to `main`; do not pull, merge, or revive old Round675, Round676, Round677, Round678, Round679, Round680, Round681, or Round682 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 962 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 57 offset 10 limit 5.
...
- Stop if not on a dedicated Round683 branch before provider work.
...
- Stop if fresh preview for shard 57 offset 10 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `300937.SZ`, `300077.SZ`, `600278.SH`, `600340.SH`, and `002632.SZ`.
...
git switch -c codex/data-pipeline-financial-timeliness-round683-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round683-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round683-20260708
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 57 --symbol-offset 10 --symbol-limit 5 --output-dir data\reports\round683_financial_statement_overlap_preview_shard57_offset10_limit5_financial_roots_20260708
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 57 --symbol-offset 10 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round683_financial_statement_shard57_offset10_limit5_20260708
```
...
- Round682 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 957 / 1,000 unique symbols using `--financial-root data\processed`.
- Shard 57 offset 5 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `000985.SZ`, `603325.SH`, `002493.SZ`, `000897.SZ`, `300003.SZ`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 192 processed rows, 91 empty requests, and 0 duplicate rows in the quality report.
- Aggregate source audit improved coverage to 962 unique symbols and 203,754 rows.
...
- Do not generate financial reporting timeliness factors from the 962-symbol cache.
