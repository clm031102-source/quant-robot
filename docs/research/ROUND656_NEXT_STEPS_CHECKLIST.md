# Round656 Next Steps Checklist

Use this after pulling latest `main`. Round656 is expected to be merged back to `main`; do not pull, merge, or revive old Round654, Round655, or Round656 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 832 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Next window: shard 51 offset 0 limit 5.
...
- Stop if not on a dedicated Round657 branch before provider work.
...
- Stop if fresh preview for shard 51 offset 0 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `600028.SH`, `600759.SH`, `300748.SZ`, `600108.SH`, and `002394.SZ`.
...
git switch -c codex/data-pipeline-financial-timeliness-round657-20260708
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round657-20260708
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round657-20260708
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 51 --symbol-offset 0 --symbol-limit 5 --output-dir data\reports\round657_financial_statement_overlap_preview_shard51_offset0_limit5_financial_roots_20260708
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 51 --symbol-offset 0 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round657_financial_statement_shard51_offset0_limit5_20260708
```
...
- Round656 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 827 / 1,000 unique symbols.
- Shard 50 offset 15 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `300142.SZ`, `300099.SZ`, `000922.SZ`, `600702.SH`, `002419.SZ`.
...
- Aggregate source audit improved coverage to 832 unique symbols and 176,095 rows.
...
- Do not generate financial reporting timeliness factors from the 832-symbol cache.
