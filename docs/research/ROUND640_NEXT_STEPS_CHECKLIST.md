# Round640 Next Steps Checklist

Use this after pulling latest `main`. Round640 is expected to be merged back to `main`; do not pull, merge, or revive old Round638, Round639, or Round640 topic branches after integration.

## Current Truth

```text
Allowed action: one small data_pipeline backfill only, after starting from merged main.
Latest coverage: 752 / 1,000 unique symbols.
Source gate: blocked by unique_symbol_count_below_minimum.
Shard 46 offset 20 is empty.
Next window: shard 47 offset 0 limit 5.
...
- Stop if not on a dedicated Round641 branch before provider work.
...
- Stop if fresh preview for shard 47 offset 0 is not exactly 5 / 5 net-new symbols.
- Stop if preview symbols are not `002811.SZ`, `002670.SZ`, `600368.SH`, `002065.SZ`, and `605499.SH`.
...
git switch -c codex/data-pipeline-financial-timeliness-round641-20260707
.\.venv\Scripts\python.exe scripts\start_task_context.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round641-20260707
.\.venv\Scripts\python.exe scripts\run_quant_pm_startup_gate.py --machine office_desktop --task data_pipeline --branch codex/data-pipeline-financial-timeliness-round641-20260707
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_overlap_preview.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 47 --symbol-offset 0 --symbol-limit 5 --output-dir data\reports\round641_financial_statement_overlap_preview_shard47_offset0_limit5_financial_roots_20260707
```

Backfill command after all gates pass:

```powershell
.\.venv\Scripts\python.exe scripts\run_financial_statement_shard_backfill.py --plan-json data\reports\round236_financial_statement_symbol_shard_plan_20260625\financial_statement_symbol_shard_plan.json --shard-id 47 --symbol-offset 0 --symbol-limit 5 --max-endpoint-requests 3000 --output-dir data\processed\round641_financial_statement_shard47_offset0_limit5_20260707
```
...
- Round640 started from clean `main` on a dedicated data-pipeline branch.
- Startup context and Quant PM startup gate passed for `office_desktop` / `data_pipeline`.
- Preflight source audit confirmed factor work remained blocked at 747 / 1,000 unique symbols.
- Shard 46 offset 15 limit 5 was confirmed as 5 / 5 net-new within financial statement roots.
- Backfilled symbols: `002327.SZ`, `603025.SH`, `600620.SH`, `302132.SZ`, `600482.SH`.
- Backfill passed with blockers `[]`, 660 endpoint requests, 221 processed rows, and 1 duplicate row in the quality report.
- Aggregate source audit improved coverage to 752 unique symbols and 159,342 rows.
...
- Do not generate financial reporting timeliness factors from the 752-symbol cache.
