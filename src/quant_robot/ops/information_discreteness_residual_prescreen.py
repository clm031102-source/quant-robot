...
STAGE = "information_discreteness_residual_prescreen"
ROUND221_SOURCE_REPORT = "docs/research/cn_stock_round221_information_discreteness_preregistration_2026-06-24.md"
NEXT_DIRECTION_WITH_LEADS = "round222_information_discreteness_cost_capacity_preflight"
NEXT_DIRECTION_WITHOUT_LEADS = "round222_rotate_after_information_discreteness_residual_failure"
FAMILY = "information_discreteness_path_quality"
SAFETY = "research_only_no_live_trading_no_broker_no_order_placement"


def build_information_discreteness_residual_prescreen(
    *,
    bars_roots: Iterable[str | Path],
    stock_basic: str | Path | pd.DataFrame | None,
    candidate_factor_names: Sequence[str] = INFORMATION_DISCRETENESS_FACTOR_NAMES,
    ...
    factor_frame = build_information_discreteness_factor_frame(
        bars,
        exposure_frame,
    ...
    result = summarize_information_discreteness_residual_prescreen(
        factor_frame,
        labels,
    ...
    result["markdown"] = render_information_discreteness_residual_prescreen_markdown(result)
    return result


def build_information_discreteness_sharded_residual_prescreen(
    *,
    bars_roots: Iterable[str | Path],
    stock_basic: str | Path | pd.DataFrame | None,
    candidate_factor_names: Sequence[str] = INFORMATION_DISCRETENESS_FACTOR_NAMES,
    ...
    _retag_information_discreteness_result(result, candidate_factor_names)
...
def build_information_discreteness_factor_frame(
    bars: pd.DataFrame,
...
def summarize_information_discreteness_residual_prescreen(
...
def _retag_information_discreteness_result(
    result: dict[str, Any],
    candidate_factor_names: Sequence[str],
) -> None:
    residual_lead_count = int(result["summary"].get("residual_research_lead_count", 0))
    ...
    result["multiple_testing_policy"] = {
        "method": "all Round221 information-discreteness candidate x horizon tests counted before any promotion claim",
        "round221_candidate_count": len(tuple(candidate_factor_names)),
    ...
    result["promotion_policy"] = {
        ...
        "reason": "Round221 residual prescreen is an IC/neutralization/reference-dedup gate only.",
    }
    result["live_boundary_allowed"] = False
    result["safety"] = SAFETY


def write_information_discreteness_residual_prescreen(output_dir: str | Path, result: dict[str, Any]) -> None:
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    (output_path / "information_discreteness_residual_prescreen.json").write_text(
        json.dumps(_sanitize(result), indent=2, sort_keys=True),
        encoding="utf-8",
    )
    (output_path / "information_discreteness_residual_prescreen.md").write_text(
        render_information_discreteness_residual_prescreen_markdown(result),
        encoding="utf-8",
    )
    _write_csv(output_path / "information_discreteness_residual_prescreen_results.csv", result.get("results", []), RESULT_COLUMNS)
    _write_csv(output_path / "information_discreteness_reference_correlations.csv", result.get("reference_correlations", []), ["lead_factor_name", *REFERENCE_CORRELATION_COLUMNS])
    _write_csv(output_path / "information_discreteness_exposure_correlations.csv", result.get("exposure_correlations", []), ["lead_factor_name", *EXPOSURE_CORRELATION_COLUMNS])
    _write_csv(output_path / "information_discreteness_raw_yearly_ic.csv", result.get("raw_yearly_ic", []), ["factor_name", *YEARLY_IC_COLUMNS])
    _write_csv(output_path / "information_discreteness_industry_neutral_yearly_ic.csv", result.get("industry_neutral_yearly_ic", []), ["factor_name", *YEARLY_IC_COLUMNS])
    _write_csv(output_path / "information_discreteness_residual_yearly_ic.csv", result.get("residual_yearly_ic", []), ["factor_name", *YEARLY_IC_COLUMNS])
    _write_csv(output_path / "information_discreteness_raw_ic_observations.csv", result.get("raw_ic_observations", []), IC_OBSERVATION_COLUMNS)
    _write_csv(output_path / "information_discreteness_industry_neutral_ic_observations.csv", result.get("industry_neutral_ic_observations", []), IC_OBSERVATION_COLUMNS)
    _write_csv(output_path / "information_discreteness_residual_ic_observations.csv", result.get("residual_ic_observations", []), IC_OBSERVATION_COLUMNS)


def render_information_discreteness_residual_prescreen_markdown(result: dict[str, Any]) -> str:
    summary = result.get("summary", {})
    lines = [
        "# Information Discreteness Residual Prescreen Round221",
        "",
    ...
    lines.extend(
        [
            ...
            "- If zero residual leads survive, the information-discreteness family must rotate instead of tuning windows.",
        ]
    )
    return "\n".join(lines) + "\n"
