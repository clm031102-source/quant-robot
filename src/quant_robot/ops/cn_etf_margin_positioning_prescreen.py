...
STAGE = "cn_etf_margin_positioning_prescreen"
SAFETY = (
    "Research-to-paper only. No portfolio grid, walk-forward, final holdout, "
    "paper signal, broker, account, order, or live trading."
)


def summarize_cn_etf_margin_positioning_prescreen(
...
def write_cn_etf_margin_positioning_prescreen(
    output_dir: str | Path,
    result: dict[str, Any],
) -> dict[str, Path]:
    ...
    paths["markdown"].write_text(render_cn_etf_margin_positioning_prescreen(result), encoding="utf-8")
    for name in paths.keys() - {"json", "markdown"}:
        _stable_frame(result.get(name, [])).to_csv(paths[name], index=False)
    return paths


def render_cn_etf_margin_positioning_prescreen(result: dict[str, Any]) -> str:
    decision = result.get("decision", {})
    lines = [
        "# CN ETF Margin-Positioning Prescreen",
        "",
        ...
        "- Bar-gap crossing windows excluded: true",
    ...
    lines.extend(
        [
            ...
            "## Boundary",
            "",
            "The diagnostic cannot rescue the primary. No sign flip, window "
            "tuning, threshold relaxation, subgroup, portfolio, walk-forward, "
            "bar-gap waiver, or holdout rescue is allowed.",
            "",
        ]
    )
    return "\n".join(lines)
...
def _stable_frame(rows: Any) -> pd.DataFrame:
    ...
    return frame.sort_values(sort, kind="stable").reset_index(drop=True) if sort and not frame.empty else frame
