...
STAGE = "official_tradeability_event_residual_prescreen"
FAMILY = "official_tradeability_event_state"
ROUND260_SOURCE_REPORT = "docs/research/cn_stock_round259_listing_age_board_structural_residual_prescreen_2026-06-26.md"
NEXT_DIRECTION_WITH_LEADS = "round261_official_tradeability_event_cost_capacity_walk_forward_preflight"
NEXT_DIRECTION_WITHOUT_LEADS = "round261_rotate_after_official_tradeability_event_state_failure"


def build_official_tradeability_event_residual_prescreen(
    *,
    bars_roots: Iterable[str | Path],
    tradeability_mask_root: str | Path,
    stock_basic: str | Path | pd.DataFrame | None,
    candidate_factor_names: Sequence[str] = OFFICIAL_TRADEABILITY_EVENT_FACTOR_NAMES,
    ...
    masks = load_official_tradeability_masks(
    ...
    stock_basic_frame = _load_stock_basic_frame(stock_basic)
    exposure_frame = build_public_trend_strength_state_exposure_frame(features, stock_basic_frame)
    factor_frame = build_official_tradeability_event_factor_frame(
        bars,
        masks,
        exposure_frame,
    ...
    result = summarize_official_tradeability_event_residual_prescreen(
        factor_frame,
        labels,
    ...
    result["tradeability_mask_root"] = str(Path(tradeability_mask_root))
    ...
    result["source_context"]["public_reference_dedup_executed"] = bool(include_reference_factors)
    result["markdown"] = render_official_tradeability_event_residual_prescreen_markdown(result)
    return result


def build_official_tradeability_event_sharded_residual_prescreen(
    *,
    bars_roots: Iterable[str | Path],
    tradeability_mask_root: str | Path,
    stock_basic: str | Path | pd.DataFrame | None,
    ...
    for shard_start, shard_end in _year_signal_shards(analysis_start, analysis_end):
        ...
        masks = load_official_tradeability_masks(
            tradeability_mask_root,
            analysis_start_date=load_start.date().isoformat(),
            analysis_end_date=shard_end.date().isoformat(),
        )
        features = build_public_trend_strength_state_bar_features(
            bars,
            horizons=tuple(horizons),
    ...
    _retag_official_tradeability_event_result(result, candidate_factor_names)
...
def load_official_tradeability_masks(
    tradeability_mask_root: str | Path,
    *,
    ...
    root = Path(tradeability_mask_root)
    dataset = root / "processed" / "tradeability_masks"
    ...
    for year in range(start.year, end.year + 1):
        files.extend((dataset / "frequency=1d" / f"market={market.upper()}" / f"year={year}").glob("*.parquet"))
    if not files:
        raise ValueError(f"No official tradeability mask parquet files found under {dataset}")
    pieces = [pd.read_parquet(path) for path in sorted(files)]
    frame = pd.concat(pieces, ignore_index=True) if pieces else pd.DataFrame()
    frame["date"] = pd.to_datetime(frame["date"], errors="coerce").dt.normalize()
    frame["market"] = frame["market"].fillna(market).astype(str).str.upper()
    return frame[(frame["market"] == market.upper()) & (frame["date"] >= start) & (frame["date"] <= end)].reset_index(drop=True)


def build_official_tradeability_event_factor_frame(
    bars: pd.DataFrame,
    ...
    *,
    ...
    factors = compute_official_tradeability_event_factors(
        bars,
        tradeability_masks,
...
def summarize_official_tradeability_event_residual_prescreen(
...
def _retag_official_tradeability_event_result(
    result: dict[str, Any],
    candidate_factor_names: Sequence[str],
) -> None:
    ...
    result["source_context"] = {
        "source_audit": ROUND260_SOURCE_REPORT,
        "candidate_family": FAMILY,
        "portfolio_grid_blocked_at_this_stage": True,
        "non_daily_basic_non_forecast_family": True,
        "uses_official_tradeability_masks": True,
        "not_proxy_limit_event_reentry": True,
        "not_listing_age_board_reentry": True,
    }
    ...
    result["multiple_testing_policy"] = {
        "method": "all Round260 official tradeability-event candidate x horizon tests counted before any promotion claim",
        "round260_candidate_count": len(tuple(candidate_factor_names)),
    ...
    result["promotion_policy"] = {
        ...
        "reason": "Round260 residual prescreen is an IC/neutralization/reference-dedup gate only.",
    }
    result["live_boundary_allowed"] = False
    result["safety"] = SAFETY


def write_official_tradeability_event_residual_prescreen(output_dir: str | Path, result: dict[str, Any]) -> None:
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    (output_path / "official_tradeability_event_residual_prescreen.json").write_text(
        json.dumps(_sanitize(result), indent=2, sort_keys=True),
        encoding="utf-8",
    )
    (output_path / "official_tradeability_event_residual_prescreen.md").write_text(
        render_official_tradeability_event_residual_prescreen_markdown(result),
        encoding="utf-8",
    )
    _write_csv(output_path / "official_tradeability_event_residual_prescreen_results.csv", result.get("results", []), RESULT_COLUMNS)
    _write_csv(output_path / "official_tradeability_event_reference_correlations.csv", result.get("reference_correlations", []), ["lead_factor_name", *REFERENCE_CORRELATION_COLUMNS])
    _write_csv(output_path / "official_tradeability_event_exposure_correlations.csv", result.get("exposure_correlations", []), ["lead_factor_name", *EXPOSURE_CORRELATION_COLUMNS])
    _write_csv(output_path / "official_tradeability_event_raw_yearly_ic.csv", result.get("raw_yearly_ic", []), ["factor_name", *YEARLY_IC_COLUMNS])
    _write_csv(output_path / "official_tradeability_event_industry_neutral_yearly_ic.csv", result.get("industry_neutral_yearly_ic", []), ["factor_name", *YEARLY_IC_COLUMNS])
    _write_csv(output_path / "official_tradeability_event_residual_yearly_ic.csv", result.get("residual_yearly_ic", []), ["factor_name", *YEARLY_IC_COLUMNS])
    _write_csv(output_path / "official_tradeability_event_raw_ic_observations.csv", result.get("raw_ic_observations", []), IC_OBSERVATION_COLUMNS)
    _write_csv(output_path / "official_tradeability_event_industry_neutral_ic_observations.csv", result.get("industry_neutral_ic_observations", []), IC_OBSERVATION_COLUMNS)
    _write_csv(output_path / "official_tradeability_event_residual_ic_observations.csv", result.get("residual_ic_observations", []), IC_OBSERVATION_COLUMNS)


def render_official_tradeability_event_residual_prescreen_markdown(result: dict[str, Any]) -> str:
    summary = result.get("summary", {})
    lines = [
        "# Official Tradeability Event Residual Prescreen Round260",
        "",
    ...
    lines.extend(
        [
            ...
            "- The family uses official A-share tradeability masks, not proxy limit events.",
            "- Residual leads only earn the right to a later cost/capacity walk-forward preflight.",
            "- If zero residual leads survive, the family must rotate instead of tuning event windows.",
        ]
    )
    return "\n".join(lines) + "\n"


def _load_stock_basic_frame(value: str | Path | pd.DataFrame | None) -> pd.DataFrame:
    ...
    return _load_stock_basic(value)
