...
FAMILY = "listing_age_board_structural_constraints"
ROUND259_SOURCE_REPORT = "docs/research/cn_stock_round256_258_three_round_review_2026-06-26.md"
NEXT_DIRECTION_WITH_LEADS = "round260_listing_age_board_cost_capacity_walk_forward_preflight"
NEXT_DIRECTION_WITHOUT_LEADS = "round260_rotate_after_listing_age_board_structural_failure"


def build_listing_age_board_residual_prescreen(
    *,
    bars_roots: Iterable[str | Path],
    stock_basic: str | Path | pd.DataFrame | None,
    candidate_factor_names: Sequence[str] = LISTING_AGE_BOARD_FACTOR_NAMES,
    ...
    factor_frame = build_listing_age_board_factor_frame(
        bars,
        stock_basic_frame,
        exposure_frame,
    ...
    result["markdown"] = render_listing_age_board_residual_prescreen_markdown(result)
    return result


def build_listing_age_board_sharded_residual_prescreen(
    *,
    bars_roots: Iterable[str | Path],
    stock_basic: str | Path | pd.DataFrame | None,
    candidate_factor_names: Sequence[str] = LISTING_AGE_BOARD_FACTOR_NAMES,
    ...
    _retag_listing_age_board_result(result, candidate_factor_names)
...
def build_listing_age_board_factor_frame(
    bars: pd.DataFrame,
    ...
    *,
    ...
    factors = compute_listing_age_board_factors(
        bars,
...
def summarize_listing_age_board_residual_prescreen(
...
def _retag_listing_age_board_result(
    result: dict[str, Any],
    candidate_factor_names: Sequence[str],
) -> None:
    ...
    result["source_context"] = {
        "source_audit": ROUND259_SOURCE_REPORT,
        "candidate_family": FAMILY,
        "portfolio_grid_blocked_at_this_stage": True,
        "non_daily_basic_non_forecast_family": True,
        "a_share_structural_constraint_family": True,
        "uses_stock_basic_list_date_delist_date_board_metadata": True,
        "not_public_technical_or_moneyflow_reentry": True,
    }
    ...
    result["multiple_testing_policy"] = {
        "method": "all Round259 listing-age/board candidate x horizon tests counted before any promotion claim",
        "round259_candidate_count": len(tuple(candidate_factor_names)),
    ...
    result["promotion_policy"] = {
        ...
        "reason": "Round259 residual prescreen is an IC/neutralization/reference-dedup gate only.",
    }
    result["live_boundary_allowed"] = False
    result["safety"] = SAFETY


def write_listing_age_board_residual_prescreen(output_dir: str | Path, result: dict[str, Any]) -> None:
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    (output_path / "listing_age_board_residual_prescreen.json").write_text(
        json.dumps(_sanitize(result), indent=2, sort_keys=True),
        encoding="utf-8",
    )
    (output_path / "listing_age_board_residual_prescreen.md").write_text(
        render_listing_age_board_residual_prescreen_markdown(result),
        encoding="utf-8",
    )
    _write_csv(output_path / "listing_age_board_residual_prescreen_results.csv", result.get("results", []), RESULT_COLUMNS)
    _write_csv(output_path / "listing_age_board_reference_correlations.csv", result.get("reference_correlations", []), ["lead_factor_name", *REFERENCE_CORRELATION_COLUMNS])
    _write_csv(output_path / "listing_age_board_exposure_correlations.csv", result.get("exposure_correlations", []), ["lead_factor_name", *EXPOSURE_CORRELATION_COLUMNS])
    _write_csv(output_path / "listing_age_board_raw_yearly_ic.csv", result.get("raw_yearly_ic", []), ["factor_name", *YEARLY_IC_COLUMNS])
    _write_csv(output_path / "listing_age_board_industry_neutral_yearly_ic.csv", result.get("industry_neutral_yearly_ic", []), ["factor_name", *YEARLY_IC_COLUMNS])
    _write_csv(output_path / "listing_age_board_residual_yearly_ic.csv", result.get("residual_yearly_ic", []), ["factor_name", *YEARLY_IC_COLUMNS])
    _write_csv(output_path / "listing_age_board_raw_ic_observations.csv", result.get("raw_ic_observations", []), IC_OBSERVATION_COLUMNS)
    _write_csv(output_path / "listing_age_board_industry_neutral_ic_observations.csv", result.get("industry_neutral_ic_observations", []), IC_OBSERVATION_COLUMNS)
    _write_csv(output_path / "listing_age_board_residual_ic_observations.csv", result.get("residual_ic_observations", []), IC_OBSERVATION_COLUMNS)


def render_listing_age_board_residual_prescreen_markdown(result: dict[str, Any]) -> str:
    summary = result.get("summary", {})
    lines = [
        "# Listing-Age / Board Structural Residual Prescreen Round259",
        "",
    ...
    lines.extend(
        [
            ...
            "- The family tests A-share listing-age and board-permission structural effects, not daily_basic, forecast, moneyflow, or public technical reentry.",
            "- Residual leads only earn the right to a later cost/capacity walk-forward preflight.",
            "- If zero residual leads survive, the family must rotate instead of tuning listing-age thresholds.",
        ]
    )
    return "\n".join(lines) + "\n"
