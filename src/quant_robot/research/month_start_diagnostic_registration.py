"""One-use identity for the fixed monthly calendar commission screen.

No permission is inherited from the policy or household studies. Artifact helpers
and the mathematical gross comparator are reused, with independent admission.
"""
...
from quant_robot.research.month_start_liquidity_diagnostic import HYPOTHESIS

STAGE = 'cn_etf_month_start_commission_diagnostic_v1'
DIRECTORY = 'data/reports/cn_etf_month_start_commission_diagnostic_20260914'
LEDGER = DIRECTORY + '/attempt_claim.json'
SOURCE_ROOT = 'data/reports/etf_monetization_20260911/calendar_liquidity_review_20260914'
...
IMPLEMENTATION_FILES = (
    'src/quant_robot/research/month_start_diagnostic_registration.py',
    'src/quant_robot/research/month_start_diagnostic_inputs.py',
    'src/quant_robot/research/month_start_diagnostic_execution.py',
    'src/quant_robot/research/month_start_diagnostic_pm_scope.py',
    'src/quant_robot/research/month_start_liquidity_diagnostic.py',
    ...
    'scripts/prepare_cn_etf_month_start_diagnostic.py',
    'scripts/run_cn_etf_month_start_diagnostic.py',
...
def expected_input_paths():
    paths = {
        'proposal': 'configs/cn_etf_month_start_liquidity_proposal_20260914.json',
...
def build_registration(*, inputs, code_files, environment, source_origin, branch):
    ...
    if not isinstance(inputs, dict) or set(inputs) != set(paths):
        raise ValueError('input roles must exactly match the 11 fixed study inputs')
    ...
    payload = {
        ...
        'window': {'first_month': '2020-01', 'last_month': '2024-05',
            'first_close': '2020-01-02', 'terminal_close': '2024-06-03',
            'cycles': 53, 'selected_transitions': 106, 'close_to_close_transitions': 1068},
        ...
        'conditional_commission_screen_only': True, 'scheduler_admission_required': True,
...
def check_scheduler_admission(scheduler, registration, *, registration_path, registration_sha256):
    ...
    expected = {'status': 'authorized_once', 'registration_id': packet['registration_id'],
        ...
        'ledger_path': LEDGER, 'conditional_commission_screen_only': True,
        'source_origin': packet['source_origin'], **{field: False for field in DENIED}}
    decision = scheduler.get('month_start_diagnostic_decision', {})
    if not isinstance(decision, dict) or canonical(decision) != canonical(expected):
        raise ValueError('dedicated month-start admission absent, consumed or mismatched')
    return expected
