"""One immutable RRR-effective study; exclusive claim precedes real signal/outcome computation."""
...
from quant_robot.research.rrr_effective_diagnostic import calculate

DIRECTORY='data/reports/positive_ev_20260921/rrr_effective_diagnostic'
REGISTRATION=DIRECTORY+'/registration.json'
PROPOSAL='configs/cn_etf_rrr_effective_diagnostic_20260921.json'
PROPOSAL_SHA256='e8ac5f25b7a63756279b7f85d9d0c2e575a5c5f02dfd1dbcfe568ffdbfe02ff1'
DECISION='rrr_effective_diagnostic_decision'
STAGE='single_rrr_effective_endpoint_cost_diagnostic'
MODE='single_rrr_effective_diagnostic_only'
ROLES={'proposal','original_proposal','reprint_scope','annual_scope','source_review','event_ledger','annual_reconciliation',
    'old_calendar','old_calendar_manifest','old_calendar_review','calendar','calendar_manifest','early_price','early_actions',
    'early_source_review','actions','price_review','distribution_2016_notice',
    *[f'annual_{y}' for y in range(2013,2020)], *[f'bars_{y}' for y in range(2020,2025)],
    *[f'prior_authority_{i}' for i in range(21)], *[f'policy_report_{y}' for y in range(2013,2025)],
    *[f'event_source_{i}' for i in range(23)]}
IMPLEMENTATION=('src/quant_robot/research/rrr_effective_study.py','src/quant_robot/research/rrr_effective_diagnostic.py',
    ...
    'scripts/run_cn_etf_rrr_effective_diagnostic.py','scripts/bootstrap.py')
...
def require_unused(root):
    if any(resolve_path(root,DIRECTORY+'/'+n).exists() for n in ('attempt_claim.json','result.json','outcome.json')):
        raise ValueError('RRR-effective hypothesis consumed; no rerun')
...
def preflight(root,scheduler,gate_supplier):
    ...
    if any(proposal[role]!=packet['inputs'][role] for role in ('original_proposal','event_ledger','annual_reconciliation')):
    ...
    if not (0<=age<=30 and gate['status']=='ready' and not gate['blockers'] and gate['mode']==MODE
            and gate['primary_market']=='CN_ETF' and safety.get('rrr_effective_diagnostic_allowed') is True
            and safety.get('rrr_effective_diagnostic_scope')==scheduler[DECISION] and safety.get('factor_batch_scope')=={}
        ...
        raise ValueError('Fresh exact RRR-effective PM gate required')
