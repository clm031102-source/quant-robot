"""One admitted historical diagnostic, retaining failure and exact input bytes."""
...
from quant_robot.research.monthly_diagnostic_registration import (
    DIRECTORY, canonical, check_scheduler_admission, claim_attempt, finish_attempt,
    resolve_path, sha256, validate_registration, verified_input_bytes,
)
from quant_robot.research.monthly_diagnostic_inputs import calculate_from_snapshots, check_source_links
...
def _check_gate(gate, packet, decision):
    try:
        ...
        valid = (0 <= age <= 30 and gate.get('status') == 'ready' and gate.get('blockers') == []
            and gate.get('mode') == 'single_monthly_diagnostic_only' and gate.get('primary_market') == 'CN_ETF'
            ...
            and safety.get('factor_batch_allowed') is False and safety.get('monthly_diagnostic_allowed') is True
            and canonical(safety.get('monthly_diagnostic_scope')) == canonical(decision)
            and safety.get('final_holdout_allowed') is False and safety.get('live_boundary_allowed') is False)
    except (KeyError, TypeError, ValueError):
        valid = False
    if not valid:
        raise ValueError('fresh dedicated Quant PM admission is required')
...
def preflight_registration(*, root, registration_path, scheduler, gate_supplier, environment):
    ...
    if resolve_path(root, packet['ledger_path']).exists():
        raise ValueError('study attempt already claimed; no automatic rerun')
...
def execute_registration(*, root, registration_path, scheduler, gate_supplier, environment, on_claim=None):
    ...
    if result_path.exists():
        raise ValueError('result artifact already exists; do not overwrite')
