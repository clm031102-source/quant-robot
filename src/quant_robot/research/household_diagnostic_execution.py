"""Run one separately admitted household study using its retained byte snapshots."""
...
from quant_robot.research.household_diagnostic_inputs import check_source_links, calculate_from_snapshots
...
def _check_gate(gate, packet, decision):
    try:
        ...
        valid = (0 <= age <= 30 and gate.get('status') == 'ready' and gate.get('blockers') == []
            and gate.get('mode') == 'single_household_diagnostic_only' and gate.get('primary_market') == 'CN_ETF'
            ...
            and safety.get('monthly_diagnostic_allowed') is False and safety.get('household_diagnostic_allowed') is True
            and canonical(safety.get('household_diagnostic_scope')) == canonical(decision)
            and safety.get('final_holdout_allowed') is False and safety.get('live_boundary_allowed') is False)
    except (KeyError, TypeError, ValueError):
        valid = False
    if not valid:
        raise ValueError('fresh dedicated household Quant PM admission is required')
