"""One-use currency-gold cadence screen; no price or cash-distribution roles."""
...
from quant_robot.research.currency_gold_cadence import calculate

DIRECTORY = 'data/reports/positive_ev_20260922/currency_gold_cadence'
REGISTRATION = DIRECTORY + '/registration.json'
DECISION = 'currency_gold_cadence_decision'
STAGE = 'single_currency_gold_cadence_screen'
MODE = 'single_currency_gold_cadence_only'
FROZEN = {
    'proposal': {'path': 'configs/cn_etf_currency_gold_proposal_20260922.json',
                 'sha256': 'e6160c95bfbeba77f1153a2be02fc6965d39f23e84c4797b8d7e6f469a631b76'},
    'source_summary': {'path': 'configs/cn_etf_currency_gold_source_review_20260922.json',
                 'sha256': '36f186c5337773d2992aea5b79d0fb78d850603befb78ca7df61ef9fe9908ff3'},
...
ROLES = {'proposal', 'source_summary', 'source_inventory', 'source_verification', 'source_scope', 'source_repair',
         'source_review', 'calendar', 'calendar_manifest', 'old_calendar',
         'old_calendar_manifest', 'old_calendar_review', *[f'source_authority_{i}' for i in range(166)]}
IMPLEMENTATION = (
    'src/quant_robot/research/currency_gold_cadence.py',
    'src/quant_robot/research/currency_gold_cadence_study.py',
    ...
    'scripts/run_cn_etf_currency_gold_cadence.py', 'scripts/bootstrap.py',
)
...
def require_unused(root):
    if any(resolve_path(root, DIRECTORY+'/'+name).exists()
        ...
        raise ValueError('Currency-gold cadence hypothesis consumed; no rerun')
...
def _source_chain(packet, snapshots):
    ...
    for role, key in [('proposal', 'economic_proposal'), ('source_scope', 'initial_source_scope'),
                      ('source_repair', 'source_format_repair'),
    ...
    if (proof.get('status') != 'passed' or proof.get('currency_changes_calculated') is not False
            or proof.get('ETF_outcomes_read') is not False or len(proof['pins']) != 166):
        raise ValueError('Pre-state original-history verification required')
    for i, original in enumerate(proof['pins']):
        # Copies preserve the exact bytes of all166original authority files;
    ...
    if (corpus['scope_sha256'] != inputs['source_repair']['sha256']
            or corpus.get('original_scope') != inputs['source_scope']
            or any(corpus.get(k) is not False for k in
                   ('currency_changes_calculated', 'signals_generated', 'ETF_outcomes_read',
                    'historical_vintage_certified', 'historical_release_time_certified'))):
...
def preflight(root, scheduler, gate_supplier):
    ...
    if not (0 <= age <= 30 and gate['status'] == 'ready' and not gate['blockers']
            ...
            and safety.get('currency_gold_cadence_allowed') is True
            and safety.get('currency_gold_cadence_scope') == scheduler[DECISION]
        ...
        raise ValueError('Fresh exact currency-gold-cadence PM gate required')
