...
def quantile_group_returns(factors: pd.DataFrame, labels: pd.DataFrame, quantiles: int = 5) -> pd.DataFrame:
    ...
    if merged.empty:
        return pd.DataFrame(columns=["date", "factor_name", "quantile", "mean_forward_return", "count"])
    merged = merged.copy()
    merged["quantile"] = merged.groupby(["date", "factor_name"], group_keys=False)["factor_value"].transform(
        lambda values: _rank_quantiles(values, quantiles)
    )
    return (
        merged.groupby(["date", "factor_name", "quantile"], as_index=False)
        .agg(mean_forward_return=("forward_return", "mean"), count=("forward_return", "size"))
        .sort_values(["date", "factor_name", "quantile"])
