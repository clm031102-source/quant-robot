"""One immutable US-variance-risk study; exclusive claim precedes real signal/outcome computation."""
...
from quant_robot.research.us_variance_risk_diagnostic import calculate

DIRECTORY='data/reports/positive_ev_20260921/us_variance_risk_diagnostic'
REGISTRATION=DIRECTORY+'/registration.json'
PROPOSAL='configs/cn_etf_us_variance_risk_diagnostic_20260921.json'
PROPOSAL_SHA256='a1d49e26eedc41d73b387a3e608e8e45a5b036e07a2752bb96f67e4167af737f'
DECISION='us_variance_risk_diagnostic_decision'
STAGE='single_us_variance_risk_gross_diagnostic'
MODE='single_us_variance_risk_diagnostic_only'
ROLES={'proposal','original_proposal','source_scope','source_review','method_html','VIX','SPX','US_calendar',
    'vix_methodology','vix_method_change','old_calendar','old_calendar_manifest','old_calendar_review',
    'calendar','calendar_manifest','early_price','early_actions','early_source_review','actions','price_review',
    'distribution_2016_notice','publication_review',
    *[f'annual_{y}' for y in range(2013,2020)], *[f'bars_{y}' for y in range(2020,2025)],
    *[f'prior_authority_{i}' for i in range(21)]}
IMPLEMENTATION=('src/quant_robot/research/us_variance_risk_study.py','src/quant_robot/research/us_variance_risk_diagnostic.py',
    ...
    'scripts/run_cn_etf_us_variance_risk_diagnostic.py','scripts/bootstrap.py')
...
def require_unused(root):
    if any(resolve_path(root,DIRECTORY+'/'+n).exists() for n in ('attempt_claim.json','result.json','outcome.json')):
        raise ValueError('US-variance-risk hypothesis consumed; no rerun')
...
def preflight(root,scheduler,gate_supplier):
    ...
    if not (0<=age<=30 and gate['status']=='ready' and not gate['blockers'] and gate['mode']==MODE
            and gate['primary_market']=='CN_ETF' and safety.get('us_variance_risk_diagnostic_allowed') is True
            and safety.get('us_variance_risk_diagnostic_scope')==scheduler[DECISION] and safety.get('factor_batch_scope')=={}
        ...
        raise ValueError('Fresh exact US-variance-risk PM gate required')
