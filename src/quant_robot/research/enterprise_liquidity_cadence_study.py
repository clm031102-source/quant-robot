"""One-use enterprise-survey cadence screen; no price or cash-distribution roles."""
...
from quant_robot.research.enterprise_liquidity_cadence import calculate

DIRECTORY = 'data/reports/positive_ev_20260922/enterprise_liquidity_cadence'
REGISTRATION = DIRECTORY + '/registration.json'
DECISION = 'enterprise_liquidity_cadence_decision'
STAGE = 'single_enterprise_liquidity_cadence_screen'
MODE = 'single_enterprise_cadence_only'
FROZEN = {
    'proposal': {'path': 'configs/cn_etf_enterprise_liquidity_proposal_20260922.json',
                 'sha256': 'c0bb5ba13b658bdb5072806ae2dbc311a80c9460d0bf33ff328e0a8364773b68'},
    'clarifications': {'path': 'configs/cn_etf_enterprise_liquidity_execution_clarifications_20260922.json',
                       'sha256': '43046f7eb2d71444488e03e9cc34b5afba741ef853d5a584766d49786b6b32f4'},
    'source_summary': {'path': 'configs/cn_etf_enterprise_liquidity_history_review_20260922.json',
                       'sha256': 'ce7d2405a9e06af5c30181309bc48739a4fc8f92af6e6628095ead2f885ec9b1'},
...
ROLES = {'proposal', 'clarifications', 'source_summary', 'source_inventory', 'source_verification',
         'source_manifest', 'source_review', 'calendar', 'calendar_manifest', 'old_calendar',
         'old_calendar_manifest', 'old_calendar_review', *[f'source_authority_{i}' for i in range(115)]}
IMPLEMENTATION = (
    'src/quant_robot/research/enterprise_liquidity_cadence.py',
    'src/quant_robot/research/enterprise_liquidity_cadence_study.py',
    ...
    'src/quant_robot/data/sources/pbc_enterprise_survey.py', 'src/quant_robot/storage/atomic.py',
    'scripts/run_cn_etf_enterprise_liquidity_cadence.py', 'scripts/bootstrap.py',
)
...
def require_unused(root):
    if any(resolve_path(root, DIRECTORY+'/'+name).exists()
        ...
        raise ValueError('Enterprise cadence hypothesis consumed; no rerun')
...
def _source_chain(packet, snapshots):
    ...
    for role, key in [('source_inventory', 'report_inventory'),
                      ('source_verification', 'source_verification'), ('source_manifest', 'source_manifest')]:
        if inputs[role] != summary[key]:
            raise ValueError('Frozen source summary and input identity differ')
    proof = json.loads(snapshots['source_verification'])
    if (proof.get('status') != 'conditional_original_history_source_review_complete'
            or proof.get('financial_states_generated') is not False
            or proof.get('returns_computed') is not False
            or proof['report_inventory_sha256'] != inputs['source_inventory']['sha256']):
        raise ValueError('Pre-state original-history verification required')
    entries = json.loads(snapshots['source_manifest'])['entries']
    if len(entries) != 115:
        raise ValueError('Exact115source authority entries required')
    for i, entry in enumerate(entries):
        expected = dict(path=entry['path'].replace('\\', '/'), sha256=entry['sha256'])
...
def preflight(root, scheduler, gate_supplier):
    ...
    if not (0 <= age <= 30 and gate['status'] == 'ready' and not gate['blockers']
            ...
            and safety.get('enterprise_cadence_allowed') is True
            and safety.get('enterprise_cadence_scope') == scheduler[DECISION]
        ...
        raise ValueError('Fresh exact enterprise-cadence PM gate required')
