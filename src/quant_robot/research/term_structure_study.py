"""One immutable Sovereign-term-slope study; exclusive claim precedes real signal/outcome computation."""
...
from quant_robot.research.term_structure_diagnostic import calculate

DIRECTORY='data/reports/positive_ev_20260921/term_structure_diagnostic'
REGISTRATION=DIRECTORY+'/registration.json'
PROPOSAL='configs/cn_etf_term_structure_diagnostic_20260922.json'
PROPOSAL_SHA256='1d77392c688ec4ee7c731df3a694454b4781fddb077b43c4c203adb5e438c697'
DECISION='term_structure_diagnostic_decision'
STAGE='single_term_structure_gross_diagnostic'
MODE='single_term_structure_diagnostic_only'
ROLES={'proposal','original_proposal','source_rows','source_verification','source_qualification','source_review',
    'old_calendar','old_calendar_manifest','old_calendar_review','calendar','calendar_manifest',
    'early_price','early_actions','early_source_review','actions','price_review','distribution_2016_notice',
    *[f'annual_{y}' for y in range(2013,2020)], *[f'bars_{y}' for y in range(2020,2025)],
    *[f'prior_authority_{i}' for i in range(21)], *[f'history_authority_{i}' for i in range(57)]}
IMPLEMENTATION=('src/quant_robot/research/term_structure_study.py','src/quant_robot/research/term_structure_diagnostic.py',
    ...
    'src/quant_robot/storage/atomic.py','scripts/run_cn_etf_term_structure_diagnostic.py','scripts/bootstrap.py')
...
def build_registration(*,inputs,code_files,branch,environment):
    ...
    if inputs['proposal']!={'path':PROPOSAL,'sha256':PROPOSAL_SHA256}:raise ValueError('Frozen sovereign-term-slope contract required')
...
def require_unused(root):
    if any(resolve_path(root,DIRECTORY+'/'+n).exists() for n in ('attempt_claim.json','result.json','outcome.json')):
        raise ValueError('Sovereign-term-slope hypothesis consumed; no rerun')
...
def preflight(root,scheduler,gate_supplier):
    ...
    contract=json.loads(snapshots['proposal'])
    for role,key in [('original_proposal','original_proposal_sha256'),('source_rows','source_rows_sha256'),
                     ('source_verification','source_verification_sha256')]:
        if packet['inputs'][role]['sha256']!=contract[key]:
            raise ValueError('Frozen source/proposal identity differs')
    verification=json.loads(snapshots['source_verification'])
    if (verification.get('status')!='verified' or verification.get('verified_daily_rows')!=2961
            or verification.get('verified_months')!=142 or verification.get('conditional_months')!=141
            or verification.get('unknown_months')!=['2020-01']
            or verification.get('no_spread_signal_or_ETF_return_computed') is not True):
        raise ValueError('Fixed historical source verification required')
    ...
    if not (0<=age<=30 and gate['status']=='ready' and not gate['blockers'] and gate['mode']==MODE
            and gate['primary_market']=='CN_ETF' and safety.get('term_structure_diagnostic_allowed') is True
            and safety.get('term_structure_diagnostic_scope')==scheduler[DECISION] and safety.get('factor_batch_scope')=={}
        ...
        raise ValueError('Fresh exact Sovereign-term-slope PM gate required')
