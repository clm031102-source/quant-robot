"""One immutable disclosed-flow study; exclusive claim precedes actual data decode."""
...
from quant_robot.research.disclosed_flow_diagnostic import calculate

DIRECTORY='data/reports/positive_ev_20260921/disclosed_flow_diagnostic'
REGISTRATION=DIRECTORY+'/registration.json'
PROPOSAL='configs/cn_etf_disclosed_flow_diagnostic_20260921.json'
PROPOSAL_SHA256='7cca44b35ee33da87b69661b0a1cea254fc6a59499abca1265e0548c8ed8743c'
DECISION='disclosed_flow_diagnostic_decision'
STAGE='single_disclosed_flow_gross_diagnostic'
MODE='single_disclosed_flow_diagnostic_only'
ROLES={'proposal','original_proposal','method_review','source_review','holdings_review','coverage_scope','coverage_result',
    'price_review','actions','calendar','calendar_manifest', *[f'bars_{y}' for y in range(2021,2025)],
    *[f'holdings_{i}' for i in range(7)],*[f'flow_{i}' for i in range(7)]}
IMPLEMENTATION=('src/quant_robot/research/disclosed_flow_study.py','src/quant_robot/research/disclosed_flow_diagnostic.py',
    ...
    'src/quant_robot/research/family_scheduler.py','src/quant_robot/paper/corporate_actions.py',
    'src/quant_robot/data/cn_calendar_snapshot.py','src/quant_robot/data/cn_trading_calendar.py',
    'src/quant_robot/storage/atomic.py','scripts/run_cn_etf_disclosed_flow_diagnostic.py','scripts/bootstrap.py')
...
def require_unused(root):
    if any(resolve_path(root,DIRECTORY+'/'+n).exists() for n in ('attempt_claim.json','result.json','outcome.json')):
        raise ValueError('Disclosed-flow hypothesis consumed; no rerun')
...
def preflight(root,scheduler,gate_supplier):
    ...
    coverage=json.loads(snapshots['coverage_result'])
    if coverage.get('scope_sha256')!=sha256(snapshots['coverage_scope']):raise ValueError('Coverage scope differs')
    ...
    if not (0<=age<=30 and gate['status']=='ready' and not gate['blockers'] and gate['mode']==MODE
            and gate['primary_market']=='CN_ETF' and safety.get('disclosed_flow_diagnostic_allowed') is True
            and safety.get('disclosed_flow_diagnostic_scope')==scheduler[DECISION] and safety.get('factor_batch_scope')=={}
        ...
        raise ValueError('Fresh exact disclosed-flow PM gate required')
