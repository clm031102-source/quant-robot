"""A separate one-use scope for the fixed month_start study, with no batch budget."""
import json
...
def month_start_diagnostic_scope(task, family_config, family_schedule, *, root, branch):
    if task != 'factor_batch':
        return None
    decision = family_config.get('month_start_diagnostic_decision')
    ...
    try:
        ...
        return None
    return {'mode': 'single_month_start_diagnostic_only', 'scope': scope}
