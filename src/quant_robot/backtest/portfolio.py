...
def select_top_n(factors: pd.DataFrame, top_n: int) -> pd.DataFrame:
    ...
    selected = (
        ...
        .sort_values(["date", "market", "factor_name", "factor_value"], ascending=[True, True, True, False])
    ...
    if selected.empty:
        ...
        return selected
    selected["target_weight"] = selected.groupby(["date", "market", "factor_name"])["asset_id"].transform(
