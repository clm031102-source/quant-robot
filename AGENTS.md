...
This profile runs safety checks, `scripts\run_desktop_factor_validation.py`, a CN data-quality audit against `data\processed` with output under `data\reports\data_quality_gap_audit_tushare_moneyflow_residual_regime`, a strict market-regime coverage check from walk-forward regime curves, the residual-regime promotion gate report, and a lightweight Markdown summary that records data-quality, promotion-gate, and regime-coverage status while rejecting mixed leaderboard/promotion case IDs. The validation step uses `configs\walk_forward_tushare_moneyflow_residual_regime.json`; the promotion gate consumes the data-quality audit and requires the market-regime coverage pack. A complete rejection set is acceptable evidence when the train/test grids complete; generated reports and processed data still stay out of Git.
