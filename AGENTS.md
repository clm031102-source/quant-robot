...
This profile runs safety checks, validates the fingerprinted Tushare SSE/SZSE calendar, builds a manifest from the tracked adjusted-ratio-clean bars authority config and moneyflow authority config, runs a stock-aware CN data-quality audit, generates a same-day frozen validation-readiness packet, runs `scripts\run_desktop_factor_validation.py`, checks market-regime coverage, builds the residual-regime promotion report, and writes a lightweight Markdown summary while rejecting mixed leaderboard/promotion case IDs. Whole-market calendar gaps are hard blockers; per-stock gaps remain review-required until suspension evidence explains them, so validation may complete while promotion remains blocked. The validation step uses `configs\walk_forward_tushare_moneyflow_residual_regime.json` with precomputed factor-matrix reuse. A complete rejection set is acceptable evidence when the train/test grids complete; generated reports and processed data still stay out of Git.
