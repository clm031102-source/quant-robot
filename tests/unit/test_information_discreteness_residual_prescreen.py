...
from quant_robot.ops.information_discreteness_residual_prescreen import (
    ...
    for signal_date in dates:
        for asset_idx in range(assets):
            ...
            reference_rows.append(
                common
                | {
                    "factor_name": "momentum_60_reference",
            ...
            for factor_name in INFORMATION_DISCRETENESS_FACTOR_NAMES:
                if factor_name == "fip_smooth_momentum_quality_60_20":
                    factor_value = true_signal + exposure_value * 0.01
                elif factor_name == "fip_discrete_jump_reversal_20_5":
...
class InformationDiscretenessResidualPrescreenTests(unittest.TestCase):
    def test_residual_prescreen_evaluates_registered_round221_candidates_without_promotion(self) -> None:
        ...
        self.assertEqual(result["source_context"]["source_audit"], "docs/research/cn_stock_round221_information_discreteness_preregistration_2026-06-24.md")
    ...
    def test_writer_outputs_structured_round221_audit_files(self) -> None:
        factor_frame, labels, reference_frame, exposure_frame = _synthetic_frames(assets=45)
        result = summarize_information_discreteness_residual_prescreen(
            factor_frame,
            labels,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp)
            write_information_discreteness_residual_prescreen(output, result)
            self.assertTrue((output / "information_discreteness_residual_prescreen.json").exists())
            self.assertTrue((output / "information_discreteness_residual_prescreen.md").exists())
            self.assertTrue((output / "information_discreteness_residual_prescreen_results.csv").exists())
            self.assertTrue((output / "information_discreteness_reference_correlations.csv").exists())
