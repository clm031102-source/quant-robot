...
class WalkForwardTests(unittest.TestCase):
    def test_walk_forward_runs_train_and_test_splits_and_writes_artifacts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("test_annualized_return", leaderboard[0])
            self.assertIn("test_win_rate", leaderboard[0])
            self.assertIn("mean_test_annualized_return", leaderboard[0])
            self.assertIn("mean_test_win_rate", leaderboard[0])
            self.assertIn("test_mean_rank_ic", leaderboard[0])
            self.assertIn("test_long_short_mean_return", leaderboard[0])
            self.assertIn("test_long_short_positive_rate", leaderboard[0])
            self.assertIn("test_turnover", leaderboard[0])
            self.assertEqual({row["execution_lag"] for row in leaderboard}, {1})
            self.assertTrue(all(row["strict_split_status"] == "pass" for row in leaderboard))
            self.assertTrue(all(row["strict_split_violations"] == 0 for row in leaderboard))
            self.assertTrue(all(row["strict_split_folds"] == 1 for row in leaderboard))
            self.assertTrue(all(pd.to_datetime(row["test_start_date"]) > pd.to_datetime(row["train_end_date"]) for row in leaderboard))
    ...
    def test_walk_forward_supports_rolling_multi_fold_validation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("mean_test_annualized_return", row)
            self.assertIn("mean_test_win_rate", row)
            self.assertIn("test_mean_rank_ic", row)
            self.assertIn("test_long_short_mean_return", row)
            self.assertIn("test_long_short_positive_rate", row)
            self.assertIn("test_turnover", row)
