...
class DisclosedFlowStudyTests(unittest.TestCase):
    def setUp(self):
        ...
        for role in study.ROLES-{'proposal', 'source_review', 'coverage_result'}:
            put(role, b'{}')
        put('proposal', (REPO/study.PROPOSAL).read_bytes(), study.PROPOSAL)
        coverage = {'scope_sha256': self.inputs['coverage_scope']['sha256']}
        put('coverage_result', canonical(coverage))
