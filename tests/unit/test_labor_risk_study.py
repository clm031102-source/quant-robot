...
class LaborRiskStudyTests(unittest.TestCase):
    def setUp(self):
        ...
        for role in study.ROLES-{'proposal', 'source_review', 'source_rows'}:
            put(role, b'{}')
        put('proposal', (REPO/study.PROPOSAL).read_bytes(), study.PROPOSAL)
        put('source_rows', canonical({'manifest_sha256':self.inputs['history_manifest']['sha256'], 'extraction_sha256':self.inputs['extraction_review']['sha256']}))
