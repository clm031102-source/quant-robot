...
from quant_robot.ops.industry_breadth_regime_residual_prescreen import (
    FAMILY,
        ...
        for asset_idx in range(assets):
            ...
            exposure_rows.append(
                ...
                | {
                    ...
                    "realized_vol_20": exposure_value * 0.2,
            ...
            for factor_name in INDUSTRY_BREADTH_REGIME_FACTOR_NAMES:
                if factor_name == "industry_breadth_repair_laggard_rebound_20":
                    factor_value = true_signal + exposure_value * 0.01
                elif factor_name == "industry_breadth_volume_confirmed_repair_20":
...
class IndustryBreadthRegimeResidualPrescreenTests(unittest.TestCase):
    def test_residual_prescreen_retags_round261_family_without_promotion(self) -> None:
        ...
        result = summarize_industry_breadth_regime_residual_prescreen(
            ...
            candidate_factor_names=INDUSTRY_BREADTH_REGIME_FACTOR_NAMES,
            horizons=(5, 20),
        ...
        self.assertTrue(result["source_context"]["not_plain_industry_leader_lag"])
        self.assertTrue(result["source_context"]["uses_industry_breadth_and_dispersion_state"])
        self.assertEqual(result["summary"]["candidate_count"], len(INDUSTRY_BREADTH_REGIME_FACTOR_NAMES))
        self.assertEqual(result["summary"]["test_count"], len(INDUSTRY_BREADTH_REGIME_FACTOR_NAMES) * 2)
    ...
    def test_writer_outputs_structured_round261_audit_files(self) -> None:
        factor_frame, labels, reference_frame, exposure_frame = _synthetic_frames(assets=45)
        result = summarize_industry_breadth_regime_residual_prescreen(
            factor_frame,
            labels,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp)
            write_industry_breadth_regime_residual_prescreen(output, result)
            self.assertTrue((output / "industry_breadth_regime_residual_prescreen.json").exists())
            self.assertTrue((output / "industry_breadth_regime_residual_prescreen.md").exists())
            self.assertTrue((output / "industry_breadth_regime_residual_prescreen_results.csv").exists())
            self.assertTrue((output / "industry_breadth_regime_reference_correlations.csv").exists())
            self.assertTrue((output / "industry_breadth_regime_residual_ic_observations.csv").exists())
