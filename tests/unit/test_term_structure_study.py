...
import pandas as pd
from quant_robot.research import term_structure_study as study
...
class TermStructureStudyTests(unittest.TestCase):
    def setUp(self):
        ...
        for role in study.ROLES-{'proposal', 'source_review'}:
            put(role, b'{}')
        verification=dict(status='verified',verified_daily_rows=2961,verified_months=142,
            conditional_months=141,unknown_months=['2020-01'],no_spread_signal_or_ETF_return_computed=True)
        put('source_verification',canonical(verification))
        contract=json.loads((REPO/study.PROPOSAL).read_bytes())
        self.assertEqual(sha256((REPO/study.PROPOSAL).read_bytes()),study.PROPOSAL_SHA256)
        for role,key in [('original_proposal','original_proposal_sha256'),('source_rows','source_rows_sha256'),
            ...
            contract[key]=self.inputs[role]['sha256']
        raw_contract=canonical(contract)
        fixed=patch.object(study,'PROPOSAL_SHA256',sha256(raw_contract));fixed.start();self.addCleanup(fixed.stop)
        put('proposal',raw_contract,study.PROPOSAL)
