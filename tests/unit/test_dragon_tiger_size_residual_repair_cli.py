...
from scripts.run_dragon_tiger_size_residual_repair import run_dragon_tiger_size_residual_repair_cli
from quant_robot.storage.dataset_store import DatasetStore


class DragonTigerSizeResidualRepairCliTests(unittest.TestCase):
    def test_cli_loads_processed_data_and_writes_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            DatasetStore(processed_root).write_frame(
                _dragon_tiger_stock_day(assets=8, event_dates=pd.bdate_range("2024-01-02", periods=5)),
            ...
            self.assertTrue((output_dir / "dragon_tiger_size_residual_repair_prescreen.json").exists())
            self.assertTrue((output_dir / "dragon_tiger_size_residual_repair_prescreen.md").exists())
            ...
            rows.append(
                {
                    ...
                    "top_list_net_amount_sum": (asset_idx % 4) * 1_000_000.0 + asset_idx,
                    ...
                    "top_inst_net_buy_sum": 10_000_000.0 + (asset_idx % 4) * 1_000_000.0 + asset_idx,
