...
from quant_robot.research import currency_gold_cadence_study as study
...
class CurrencyGoldCadenceAdmissionTests(unittest.TestCase):
    def setUp(self):
        ...
        put('source_inventory', canonical(dict(scope_sha256=self.inputs['source_repair']['sha256'],original_scope=self.inputs['source_scope'],
        ...
        put('source_verification', canonical(dict(status='passed', currency_changes_calculated=False,
            ETF_outcomes_read=False, pins=[dict(path=f'original/{i}',
                sha256=self.inputs[f'source_authority_{i}']['sha256']) for i in range(166)])))
        summary = {'pins': {key: self.inputs[role] for role, key in
                   [('proposal', 'economic_proposal'), ('source_scope', 'initial_source_scope'),
    ...
    def test_gate_allows_only_exact_cadence_and_preflight_does_not_calculate(self):
        ...
        self.assertTrue(gate['safety']['currency_gold_cadence_allowed'])
    ...
    def test_multiple_dedicated_scopes_fail_closed(self):
        ...
        self.assertFalse(gate['safety']['currency_gold_cadence_allowed'])
        self.assertFalse(gate['safety']['factor_batch_allowed'])
