...
class GuiDesktopAppTests(unittest.TestCase):
    ...
    def test_daily_page_exposes_trading_system_blueprint_panel(self):
        ...
        self.assertIn("daily-ops-paper-recheck-status", html)
        self.assertIn("daily-ops-paper-recheck-table", html)
        self.assertIn("renderDailyOpsHandoff", app_js)
        self.assertIn("renderDailyOpsPaperExecutionRecheck", app_js)
        self.assertIn("daily_ops_handoff", app_js)
        self.assertIn("paper_execution_recheck", app_js)
        self.assertIn("dailyOpsPaperRecheckRows", app_js)
        self.assertIn("纸面工单可观察", app_js)
        self.assertIn("dailyOpsPaperTicketRows", app_js)
        self.assertIn("Daily Ops 纸面工单", app_js)
        self.assertIn("paper_observation_only", app_js)
        ...
        self.assertIn("recalculated_commission_cost_at_external_price", app_js)
        self.assertIn("recalculated_cash_required_at_external_price", app_js)
        self.assertIn("recalculated_cash_released_at_external_price", app_js)
        self.assertIn("externalCash < recalculatedCashRequired", app_js)
        self.assertIn('data-side="${escapeRawHtml(item.side || "")}"', app_js)
        self.assertIn("effective_target_value_for_recalculation", app_js)
        self.assertIn("small_capital_recheck_budget_applied", app_js)
        self.assertIn("data-small-capital-capped-notional", app_js)
        ...
        self.assertIn("row_action_code", beginner_execution_block)
        self.assertIn("row_action_label", beginner_execution_block)
        self.assertIn("plain_row_instruction", beginner_execution_block)
        self.assertIn("逐行结论=", beginner_execution_block)
...
class GuiSnapshotTests(unittest.TestCase):
    ...
    def test_daily_trade_advisory_exposes_selected_candidate_alias(self):
        ...
        self.assertIn("cash_feasibility", snapshot["pretrade_readiness"])
        self.assertIn("manual_available_cash", snapshot["pretrade_readiness"]["summary"])
        self.assertIn("manual_available_cash", snapshot["manual_broker_handoff"]["summary"])
        self.assertIn("signal_freshness", {item["check_id"] for item in snapshot["pretrade_readiness"]["required_confirmations"]})
        self.assertIn("manual_available_cash", {item["check_id"] for item in snapshot["pretrade_readiness"]["required_confirmations"]})
    ...
    def test_daily_trade_advisory_exposes_daily_ops_handoff_for_paper_ready_ticket(self):
        ...
        self.assertEqual(handoff["summary"]["beginner_live_handoff_mode"], "paper_observation_only")
        self.assertEqual(handoff["summary"]["beginner_ticket_source"], "daily_ops")
        ...
        recheck = handoff["paper_execution_recheck"]
        self.assertEqual(recheck["stage"], "phase_6_34_daily_ops_paper_execution_recheck")
        self.assertEqual(recheck["summary"]["recheck_mode"], "manual_price_cash_required")
        self.assertEqual(recheck["summary"]["ticket_count"], 1)
        self.assertTrue(recheck["summary"]["broker_realtime_price_required"])
        self.assertTrue(recheck["summary"]["manual_available_cash_required"])
        self.assertTrue(recheck["summary"]["skip_if_price_outside_guardrail"])
        self.assertFalse(recheck["summary"]["order_placement_allowed"])
        self.assertFalse(recheck["summary"]["auto_order_allowed"])
        self.assertIn("broker_price_recheck_local_calculator", recheck["summary"]["local_decision_engine"])
        self.assertEqual(recheck["rows"][0]["asset_id"], "CN_ETF_XSHG_501222")
        self.assertEqual(recheck["rows"][0]["side"], "buy")
        self.assertTrue(recheck["rows"][0]["broker_realtime_price_required"])
        self.assertTrue(recheck["rows"][0]["manual_available_cash_required"])
        self.assertTrue(recheck["rows"][0]["skip_if_price_outside_guardrail"])
        self.assertAlmostEqual(recheck["rows"][0]["max_reference_price_deviation_pct"], 0.005)
        self.assertEqual(recheck["rows"][0]["max_slippage_bps"], 10)
        self.assertEqual(recheck["rows"][0]["board_lot_size"], 100)
        self.assertFalse(recheck["rows"][0]["order_placement_allowed"])
        self.assertFalse(recheck["rows"][0]["auto_order_allowed"])
                ...
                json.dumps(
                    {
                        ...
                        "coverage": {"latest_data_date": "2026-01-13"},
                    }
                ),
                encoding="utf-8",
            )
    ...
    def test_control_center_snapshot_exposes_work_backtest_method_and_safety(self):
        ...
        self.assertIn("portfolio_value", authority_rows["daily_trade_advisory"]["comparison_keys"])
        self.assertIn("risk_profile_id", authority_rows["daily_trade_advisory"]["comparison_keys"])
        ...
        self.assertEqual(authority_rows["daily_trade_advisory"]["canonical_request"]["risk_profile_id"], "balanced_20dd")
    ...
    def test_control_center_promotes_only_clean_server_closure_streak_to_small_capital_candidate(self):
        ...
        master_gate = control["pre_live_master_gate"]
        ...
        self.assertEqual(master_gate["stage"], "gui_pre_live_master_gate")
        self.assertEqual(master_gate["summary"]["status"], "manual_small_capital_observation_ready")
        self.assertEqual(master_gate["summary"]["decision"], "external_manual_small_capital_observation_only")
        self.assertTrue(master_gate["summary"]["manual_small_capital_observation_allowed"])
        self.assertFalse(master_gate["summary"]["software_order_submission_allowed"])
        self.assertFalse(master_gate["summary"]["order_placement_allowed"])
        self.assertFalse(master_gate["summary"]["broker_connection_allowed"])
        self.assertFalse(master_gate["summary"]["account_read_allowed"])
        self.assertEqual(master_gate["summary"]["passed_gate_count"], master_gate["summary"]["required_gate_count"])
        self.assertIn("same_parameter_paper_evidence", {row["gate_id"] for row in master_gate["rows"]})
        self.assertIn("paper_performance_quality", {row["gate_id"] for row in master_gate["rows"]})
        self.assertIn("live_boundary", {row["gate_id"] for row in master_gate["rows"]})
        ...
        self.assertEqual(gate["summary"]["paper_total_trade_count"], 40)
        self.assertEqual(gate["summary"]["paper_trade_count_quality_status"], "pass")
        ...
        self.assertEqual(scorecard["summary"]["paper_total_trade_count"], 40)
    ...
    def test_tiny_trade_count_paper_streak_blocks_small_capital_candidate(self):
        from quant_robot.gui.control_center import build_control_center_snapshot
        from quant_robot.gui.operation_ledger import append_operation_ledger_entry

        def append_tiny_trade_count_day(root: Path, day: str) -> None:
            paper_request = {
                "market": "CN_ETF",
                "factor_name": "momentum_2",
                "top_n": 2,
            ...
            append_operation_ledger_entry(
                ...
                status="completed",
                request=paper_request,
                result={"metrics": {"total_return": 0.03, "max_drawdown": -0.01, "win_rate": 0.60, "trade_count": 1}},
            )
            append_operation_ledger_entry(
                repo_root=root,
                workflow_id="post_close_journal",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            for day in ["2026-06-24", "2026-06-25", "2026-06-26", "2026-06-29", "2026-06-30"]:
                append_tiny_trade_count_day(root, day)
        ...
        gate = control["server_capital_observation_gate"]
        self.assertEqual(ledger["summary"]["paper_positive_days"], 5)
        self.assertEqual(ledger["summary"]["paper_average_win_rate"], 0.60)
        self.assertEqual(ledger["summary"]["paper_total_trade_count"], 5)
        self.assertEqual(ledger["summary"]["paper_min_trade_count"], 30)
        self.assertEqual(ledger["summary"]["paper_trade_count_quality_status"], "blocked")
        self.assertFalse(ledger["summary"]["paper_performance_quality_passed"])
        self.assertEqual(gate["summary"]["status"], "blocked_need_paper_performance_quality")
        gate_rows = {row["gate_id"]: row for row in gate["rows"]}
        self.assertEqual(gate_rows["paper_performance_quality"]["status"], "blocked")
        self.assertIn("trade_count=5/30", gate_rows["paper_performance_quality"]["evidence"])
        scorecard = gate["evidence_scorecard"]
        self.assertEqual(scorecard["summary"]["paper_total_trade_count"], 5)
        self.assertEqual(scorecard["summary"]["paper_min_trade_count"], 30)
        self.assertFalse(scorecard["summary"]["manual_observation_material_ready"])
        packet = gate["manual_observation_packet"]
        self.assertFalse(packet["summary"]["manual_observation_material_ready"])
    ...
    def test_gui_browser_smoke_script_writes_evidence_packet(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                self.assertIn("pre_live_master_gate_panel", check_ids)
                ...
                self.assertIn("small_capital_budget", smoke_source)
                self.assertIn("small_capital_capped_notional", smoke_source)
                self.assertIn("small_capital_limit_breached", smoke_source)
...
class GuiHttpTests(unittest.TestCase):
    def test_http_app_serves_index_snapshot_and_demo_workflows(self):
        ...
        try:
            ...
            self.assertIn("ordinary-trading-cockpit", html)
            self.assertIn("ordinary-trading-cockpit-summary", html)
            self.assertIn("ordinary-trading-cockpit-top3", html)
            self.assertIn("ordinary-trading-cockpit-gates", html)
            self.assertIn("ordinary-trading-cockpit-actions", html)
            ...
            self.assertIn("daily-current-position-actions", html)
            self.assertIn("data-current-position-action", html)
            self.assertIn("纸面空仓试算", html)
            ...
            self.assertIn("small_capital_budget", app_js)
            self.assertIn("small_capital_capped_notional", app_js)
            self.assertIn("small_capital_limit_breached", app_js)
            self.assertIn("small_capital_quantity_at_reference", app_js)
            self.assertIn("small_capital_ticket_limit_breach_count", app_js)
            self.assertIn("small_capital_total_capped_notional", app_js)
            ...
            self.assertIn("small_capital_manual_observation_plan", app_js)
            self.assertIn("daily-manual-observation-small-capital-plan", html)
            self.assertIn("renderDailySmallCapitalManualObservationPlan", app_js)
            self.assertIn("external_manual_small_capital_observation_candidate", app_js)
            self.assertIn("data-small-capital-observation-target", app_js)
            ...
            self.assertIn("renderOrdinaryTradingCockpit", app_js)
            self.assertIn("ordinaryTradingCockpitRows", app_js)
            self.assertIn("ordinaryTradingCockpitTop3Rows", app_js)
            self.assertIn("ordinaryTradingCockpitActionRows", app_js)
            self.assertIn("today_operation_action_code", app_js)
            self.assertIn("today_operation_quantity", app_js)
            self.assertIn("today_operation_skip_rules", app_js)
            self.assertIn("today_operation_post_close_record", app_js)
            self.assertIn("broker_price_outside_guardrail", app_js)
            self.assertIn("blocked_fix_current_positions", app_js)
            self.assertIn("current_positions_missing", app_js)
            self.assertIn("fallback_baseline_not_tradeable", app_js)
            self.assertIn("candidate_trade_evidence_incomplete", app_js)
            self.assertIn("ordinaryTradingCockpitBlockerSkipRules", app_js)
            self.assertIn("record_post_close_journal", app_js)
            self.assertIn("same_parameter_top3_status", app_js)
            self.assertIn("post_close_journal_status", app_js)
            self.assertIn("manual_review_candidate", app_js)
            self.assertIn("renderOrdinaryTradingCockpit();", app_js)
            ...
            self.assertIn("renderDailyCandidateEvidenceRepairPlan", app_js)
            self.assertIn("candidate_evidence_repair_plan", app_js)
            ...
            self.assertIn("dailySameParameterPaperStatus", app_js)
            ...
            self.assertIn("daily-manual-available-cash", html)
            self.assertIn("manual_available_cash: valueOf(\"daily-manual-available-cash\")", app_js)
            self.assertIn("现金闸门", app_js)
            self.assertIn("manual_cash_shortfall", app_js)
            self.assertIn("manual_cash_not_provided", app_js)
            self.assertIn('cashStatus === "not_provided" ? "danger"', app_js)
            self.assertIn("不能进入人工券商复核", app_js)
            ...
            self.assertIn("renderDailyCurrentPositionActions", app_js)
            self.assertIn("applyPaperFlatPositionTemplate", app_js)
            self.assertIn("sanitizeCurrentPositionInput", app_js)
            self.assertIn("paper_flat_position_template", app_js)
            self.assertIn("explicit_current_positions_required", app_js)
            self.assertIn("current_position_empty_blocks_ticket_generation", app_js)
            self.assertIn("current_position_forbidden_field", app_js)
            self.assertIn('if (workflowId === "daily_trade_advisory") return buildDailyTradeAdvisoryParams();', app_js)
            self.assertIn("attachRequestToResult", app_js)
            self.assertIn("manual_available_cash: params.get(\"manual_available_cash\")", app_js)
            self.assertIn("risk_profile_id: params.get(\"risk_profile_id\")", app_js)
            ...
            self.assertIn("const paperStatus = dailySameParameterPaperStatus();", command_rail_block)
            self.assertIn("manualReviewGate.allowed", command_rail_block)
            self.assertIn("paperStatus.ready", command_rail_block)
            ...
            self.assertIn("paperStatus.ready", trade_system_evidence_block)
            ...
            self.assertIn("control-pre-live-master-gate", html)
            self.assertIn("pre_live_master_gate", app_js)
            self.assertIn("renderPreLiveMasterGate", app_js)
            self.assertIn("manual_small_capital_observation_allowed", app_js)
            self.assertIn("external_manual_small_capital_observation_only", app_js)
            ...
            self.assertIn("small_capital_limit_breached", app_js)
            self.assertIn("small_capital_excess_notional", app_js)
            self.assertIn("small_capital_max_quantity_at_reference", app_js)
            self.assertIn("small_capital_quantity_match_allowed", app_js)
            self.assertIn("quantity_plan_basis", app_js)
            ...
            self.assertIn("estimated_commission_bps", app_js)
            self.assertIn("estimated_commission_cost", app_js)
            self.assertIn("estimated_total_execution_cost", app_js)
            self.assertIn("estimated_total_execution_cost_bps", app_js)
            ...
            self.assertIn("const paperStatus = dailySameParameterPaperStatus();", evidence_chain_block)
            self.assertIn("manualReviewGate.allowed", evidence_chain_block)
            self.assertIn("paperStatus.ready", evidence_chain_block)
            ...
            self.assertIn('"estimated_buy_cash_required"', daily_manual_table_block)
            self.assertIn('"estimated_sell_cash_released"', daily_manual_table_block)
            self.assertIn('"estimated_cash_impact_after_costs"', daily_manual_table_block)
            ...
            self.assertIn("最终动作=", execution_receipts_block)
            self.assertIn("最终票据=", execution_receipts_block)
            self.assertIn("次日隔离=", execution_receipts_block)
            self.assertIn("final_action_status", execution_receipts_block)
            self.assertIn("final_manual_ticket_count", execution_receipts_block)
            self.assertIn("final_next_session_quarantine_required", execution_receipts_block)
            ...
            self.assertIn("manual_available_cash: summary.manual_available_cash", daily_receipt_block)
            self.assertIn("manual_cash_feasibility_status: summary.manual_cash_feasibility_status", daily_receipt_block)
            self.assertIn("manual_cash_shortfall: summary.manual_cash_shortfall", daily_receipt_block)
            self.assertIn("risk_profile_id: summary.risk_profile_id", daily_receipt_block)
