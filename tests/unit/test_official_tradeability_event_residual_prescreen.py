...
from quant_robot.ops.official_tradeability_event_residual_prescreen import (
    FAMILY,
    NEXT_DIRECTION_WITHOUT_LEADS,
    STAGE,
    summarize_official_tradeability_event_residual_prescreen,
    write_official_tradeability_event_residual_prescreen,
)
...
def _synthetic_frames(*, assets: int = 54) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for signal_date in dates:
        for asset_idx in range(assets):
            ...
            exposure_value = float((asset_idx * 19) % assets)
            common = {
                "date": signal_date,
            ...
            exposure_rows.append(
                ...
                | {
                    "industry": industry,
                    "amount": 50_000_000.0,
                    "adv20_amount": 55_000_000.0 + exposure_value * 10_000.0,
            ...
            reference_rows.append(
                ...
                | {
                    ...
                    "factor_value": float((asset_idx * 11) % assets),
                    "amount": 50_000_000.0,
                    "adv20_amount": 50_000_000.0,
                }
            )
            for factor_name in OFFICIAL_TRADEABILITY_EVENT_FACTOR_NAMES:
                if factor_name == "official_limit_down_reopen_rebound_5":
                    factor_value = true_signal + exposure_value * 0.01
                elif factor_name == "official_tradeability_cleanliness_20":
                    factor_value = true_signal * 0.8 + float((asset_idx * 5) % assets) * 0.02
                else:
                    factor_value = float((asset_idx * (len(factor_name) % 13 + 3)) % assets)
                factor_rows.append(
                    common
                    | {
...
class OfficialTradeabilityEventResidualPrescreenTests(unittest.TestCase):
    def test_residual_prescreen_retags_round260_family_without_promotion(self) -> None:
        ...
        result = summarize_official_tradeability_event_residual_prescreen(
            ...
            candidate_factor_names=OFFICIAL_TRADEABILITY_EVENT_FACTOR_NAMES,
            horizons=(5, 20),
        ...
        self.assertTrue(result["source_context"]["uses_official_tradeability_masks"])
        self.assertEqual(result["summary"]["candidate_count"], len(OFFICIAL_TRADEABILITY_EVENT_FACTOR_NAMES))
        self.assertEqual(result["summary"]["test_count"], len(OFFICIAL_TRADEABILITY_EVENT_FACTOR_NAMES) * 2)
    ...
    def test_writer_outputs_structured_round260_audit_files(self) -> None:
        factor_frame, labels, reference_frame, exposure_frame = _synthetic_frames(assets=45)
        result = summarize_official_tradeability_event_residual_prescreen(
            factor_frame,
            labels,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp)
            write_official_tradeability_event_residual_prescreen(output, result)
            self.assertTrue((output / "official_tradeability_event_residual_prescreen.json").exists())
            self.assertTrue((output / "official_tradeability_event_residual_prescreen.md").exists())
            self.assertTrue((output / "official_tradeability_event_residual_prescreen_results.csv").exists())
            self.assertTrue((output / "official_tradeability_event_reference_correlations.csv").exists())
            self.assertTrue((output / "official_tradeability_event_residual_ic_observations.csv").exists())
