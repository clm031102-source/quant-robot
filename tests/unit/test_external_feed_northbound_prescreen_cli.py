...
from tests.unit.test_external_feed_northbound_prescreen import (
    _synthetic_bars,
    _synthetic_hk_hold,
    _synthetic_hsgt_flow,
)


class ExternalFeedNorthboundPrescreenCliTests(unittest.TestCase):
    def test_cli_runner_loads_processed_external_feeds_and_writes_outputs(self) -> None:
        from scripts.run_external_feed_northbound_prescreen import run_external_feed_northbound_prescreen_cli
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            DatasetStore(external_root).write_frame(
                _synthetic_hk_hold(bars, raw_dates=raw_dates),
                "processed/external_hk_hold",
                {"frequency": "1d", "market": "CN", "year": "2024"},
            )
            DatasetStore(external_root).write_frame(
                _synthetic_hsgt_flow(raw_dates),
            ...
            payload = json.loads((output / "external_feed_northbound_prescreen.json").read_text(encoding="utf-8"))
