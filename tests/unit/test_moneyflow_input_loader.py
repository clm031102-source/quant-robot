...
class MoneyflowInputLoaderTests(unittest.TestCase):
    ...
    def test_loader_accepts_authority_config_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config = root / "authority_moneyflow_inputs.json"
            config.write_text(
                json.dumps(
                    {
                        "market": "CN",
            ...
            result = load_moneyflow_inputs(config, "CN")

            self.assertEqual(pd.to_datetime(result["date"]).dt.strftime("%Y-%m-%d").tolist(), ["2024-01-02", "2024-01-03", "2024-01-04"])

    def test_loader_rejects_duplicate_authority_moneyflow_input_keys(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            first = root / "first"
            second = root / "second"
            DatasetStore(first).write_frame(_frame("2024-01-02"), "processed/moneyflow_inputs", {"frequency": "1d", "market": "CN", "year": "2024"})
            DatasetStore(second).write_frame(_frame("2024-01-02"), "processed/moneyflow_inputs", {"frequency": "1d", "market": "CN", "year": "2024"})
            config = root / "authority_moneyflow_inputs.json"
            config.write_text(
            ...
            with self.assertRaisesRegex(ValueError, "duplicate authority processed/moneyflow_inputs"):
                load_moneyflow_inputs(config, "CN")
