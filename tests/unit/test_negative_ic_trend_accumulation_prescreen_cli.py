...
from scripts.run_negative_ic_trend_accumulation_prescreen import (
    run_negative_ic_trend_accumulation_prescreen_cli,
)


class NegativeIcTrendAccumulationPrescreenCliTests(unittest.TestCase):
    def test_cli_writes_json_markdown_result_csv_and_ic_csv(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((output / "negative_ic_trend_accumulation_prescreen.json").exists())
            self.assertTrue((output / "negative_ic_trend_accumulation_prescreen.md").exists())
            self.assertTrue((output / "negative_ic_trend_accumulation_prescreen_results.csv").exists())
            self.assertTrue((output / "negative_ic_trend_accumulation_prescreen_ic_observations.csv").exists())
            payload = json.loads(
                (output / "negative_ic_trend_accumulation_prescreen.json").read_text(encoding="utf-8")
            )
