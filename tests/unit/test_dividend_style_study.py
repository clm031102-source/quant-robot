...
import pandas as pd
from quant_robot.research import dividend_style_study as study
...
class DividendStyleStudyTests(unittest.TestCase):
    def setUp(self):
        ...
        identity={k:proposal['source_scope'][k] for k in ('url','params')}
        put('price_claim',canonical(dict(**identity,scope_sha256=sha256(raw_proposal))))
        put('price_receipt',canonical(dict(**identity,status='retained_current_source_version',
            http_status=200,bytes=2,sha256=sha256(b'{}'))))
    ...
    def test_source_ledger_cannot_be_swapped_through_new_registration(self):
        replacement=b'{"replacement":true}'
        (self.root/self.inputs['source_ledger']['path']).write_bytes(replacement)
        self.packet['inputs']['source_ledger']['sha256']=sha256(replacement)
