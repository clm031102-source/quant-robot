...
from scripts.run_profitability_quality_factor_matrix_smoke import (
    run_profitability_quality_factor_matrix_smoke_cli,
)
...
class ProfitabilityQualityFactorMatrixSmokeCliTests(unittest.TestCase):
    def test_cli_writes_json_markdown_and_candidate_csv(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((output / "profitability_quality_factor_matrix_smoke.json").exists())
            self.assertTrue((output / "profitability_quality_factor_matrix_smoke.md").exists())
            self.assertTrue((output / "profitability_quality_factor_matrix_candidate_summary.csv").exists())
            payload = json.loads((output / "profitability_quality_factor_matrix_smoke.json").read_text(encoding="utf-8"))
