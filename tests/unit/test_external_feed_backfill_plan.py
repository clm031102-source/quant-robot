...
class ExternalFeedBackfillPlanTests(unittest.TestCase):
    def test_builds_monthly_shards_with_endpoint_call_estimates_and_commands(self):
        ...
        first = plan["shards"][0]
        expected_output = str(Path("data/processed/external_feed_backfill_round172"))
        ...
        self.assertEqual(first["processed_output_dir"], expected_output)
        self.assertIn(f"--output-dir {expected_output}", first["command"])
