"""Preflight or run one frozen conditional disclosed-flow gross diagnostic."""
import argparse
import json
...
from quant_robot.research import disclosed_flow_study as study
...
def run(root, *, execute=False):
    ...
    if not execute:
        ...
        return {'status': 'ready_unconsumed', 'input_files': len(snapshots),
            'registration_id': packet['registration_id'], 'prices_decoded': False, 'factor_generated': False}
    result = study.execute(root, scheduler, gate_supplier)
    return {'status': 'completed', 'diagnostic': result['diagnostic'], 'result_path': study.DIRECTORY+'/result.json'}
