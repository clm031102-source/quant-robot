...
from quant_robot.factors.official_tradeability_events import OFFICIAL_TRADEABILITY_EVENT_FACTOR_NAMES  # noqa: E402
from quant_robot.ops.capacity_safe_price_volume_prescreen import (  # noqa: E402
    DEFAULT_ANALYSIS_END_DATE,
    DEFAULT_ANALYSIS_START_DATE,
)
from quant_robot.ops.official_tradeability_event_residual_prescreen import (  # noqa: E402
    build_official_tradeability_event_residual_prescreen,
    build_official_tradeability_event_sharded_residual_prescreen,
...
DEFAULT_OUTPUT_DIR = Path("data/reports/official_tradeability_event_residual_prescreen_round260_20260626")


def run_official_tradeability_event_residual_prescreen_cli(
    *,
    ...
    if factor_frame is not None and labels is not None:
        ...
        result = build_official_tradeability_event_sharded_residual_prescreen(
            bars_roots=tuple(Path(path) for path in bars_roots),
            tradeability_mask_root=Path(tradeability_mask_root),
        ...
        result = build_official_tradeability_event_residual_prescreen(
            bars_roots=tuple(Path(path) for path in bars_roots),
            tradeability_mask_root=Path(tradeability_mask_root),
            stock_basic=Path(stock_basic) if isinstance(stock_basic, (str, Path)) else stock_basic,
            candidate_factor_names=candidate_factor_names,
    ...
    write_official_tradeability_event_residual_prescreen(output_dir, result)
    return result


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Round260 official tradeability-event residual IC and reference de-duplication prescreen."
    )
    ...
    result = run_official_tradeability_event_residual_prescreen_cli(
        ...
        candidate_factor_names=tuple(args.candidate_factor_names or OFFICIAL_TRADEABILITY_EVENT_FACTOR_NAMES),
