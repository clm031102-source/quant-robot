...
from quant_robot.ops.shortlist_public_factor_entry_filter import (  # noqa: E402
...
DEFAULT_OUTPUT_DIR = Path("data/reports/shortlist_public_factor_entry_filter")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Project public-factor selected-entry cash filters onto a frozen official event-return template."
    )
    ...
    parser.add_argument("--pre-exclude-candidate", action="append", default=[])
    parser.add_argument("--dragon-tiger-source", default=None)
    ...
    parser.add_argument("--max-missing-factor-share", type=float, default=0.20)
    ...
    audit = build_public_factor_entry_filter_audit(
        ...
        candidates=tuple(parse_public_factor_filter_spec(value) for value in args.candidate),
        pre_exclude_candidates=tuple(args.pre_exclude_candidate),
        dragon_tiger_source=Path(args.dragon_tiger_source) if args.dragon_tiger_source else None,
        ...
        max_missing_factor_share=args.max_missing_factor_share,
    ...
    write_public_factor_entry_filter_audit(Path(args.output_dir), audit)
    print(
        json.dumps(
            {
