...
DEFAULT_OUTPUT_DIR = Path("data/reports/round192_external_margin_credit_prescreen_20260623")


def run_external_feed_margin_credit_prescreen_cli(
    *,
    ...
    write_external_feed_margin_credit_prescreen(output_dir, result)
    return result


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Round192 CN stock external margin-credit IC/quantile/turnover prescreen."
    )
    ...
    parser.add_argument("--horizons", default=",".join(str(horizon) for horizon in DEFAULT_MARGIN_CREDIT_HORIZONS))
