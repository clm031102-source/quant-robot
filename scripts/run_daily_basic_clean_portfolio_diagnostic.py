...
from quant_robot.ops.daily_basic_clean_portfolio_diagnostic import (  # noqa: E402
    DEFAULT_CANDIDATES,
    run_daily_basic_clean_portfolio_diagnostic,
)
...
DEFAULT_OUTPUT_DIR = Path("data/reports/daily_basic_clean_portfolio_diagnostic")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run a clean-universe close-price portfolio diagnostic for daily-basic public factors."
    )
    ...
    parser.add_argument("--market-regime-lookback", type=int)
    parser.add_argument("--min-market-momentum", type=float, default=0.0)
    parser.add_argument("--max-market-drawdown", type=float)
    args = parser.parse_args()

    result = run_daily_basic_clean_portfolio_diagnostic(
        ...
        market_regime_lookback=args.market_regime_lookback,
        min_market_momentum=args.min_market_momentum,
        max_market_drawdown=args.max_market_drawdown,
    )
    print(
        json.dumps(
            {
