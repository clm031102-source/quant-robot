...
from quant_robot.ops.financial_pit_post_announcement_gap_reversal_preregistration import (  # noqa: E402
...
DEFAULT_CANDIDATE_SEED = Path("configs/family_rotation_seed_round223_financial_pit_post_announcement_gap_reversal_20260624.json")
DEFAULT_OUTPUT_DIR = Path("data/reports/financial_pit_post_announcement_gap_reversal_preregistration_round223_20260624")


def run_financial_pit_post_announcement_gap_reversal_preregistration_cli(
    *,
    ...
    if not allow_not_ready and not result["summary"].get("passes", False):
        ...
        raise RuntimeError(f"Financial PIT post-announcement gap reversal preregistration is not ready: {blockers}")
    return result


def main() -> None:
    parser = argparse.ArgumentParser(description="Run Round223 financial PIT post-announcement gap reversal preregistration.")
