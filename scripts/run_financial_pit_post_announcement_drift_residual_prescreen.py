...
from quant_robot.ops.financial_pit_post_announcement_drift_residual_prescreen import (  # noqa: E402
    build_financial_pit_post_announcement_drift_residual_prescreen,
    write_financial_pit_post_announcement_drift_residual_prescreen,
)
...
DEFAULT_PREREGISTRATION_JSON = Path(
    ...
    "financial_pit_post_announcement_drift_preregistration.json"
)
DEFAULT_OUTPUT_DIR = Path("data/reports/financial_pit_post_announcement_drift_residual_prescreen_round222_20260624")


def run_financial_pit_post_announcement_drift_residual_prescreen_cli(
    *,
    ...
    write_financial_pit_post_announcement_drift_residual_prescreen(output_dir, result)
    if not allow_not_ready and not result["summary"].get("passes", False):
        blockers = ", ".join(result["summary"].get("blockers", []) or [])
        raise RuntimeError(f"Financial PIT post-announcement drift residual prescreen is not ready: {blockers}")
    return result


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Round222 financial PIT post-announcement drift residual IC and neutralization prescreen."
    )
    ...
    result = run_financial_pit_post_announcement_drift_residual_prescreen_cli(
