"""Preview or archive a bounded prospective moneyflow source receipt."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    status = result.get("previous_status") if result["status"] == "already_attempted" else result["status"]
    return 0 if status in {"preview", "not_due", "observed_unqualified"} else 1
