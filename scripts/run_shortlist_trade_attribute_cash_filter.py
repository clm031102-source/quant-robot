...
DEFAULT_OUTPUT_DIR = Path("data/reports/shortlist_trade_attribute_cash_filter")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Project selected-trade attribute cash filters onto a frozen official event-return template."
    )
    ...
    audit = build_trade_attribute_cash_filter_audit(
        ...
        candidates=tuple(parse_attribute_filter_spec(value) for value in args.candidate),
    ...
    write_trade_attribute_cash_filter_audit(Path(args.output_dir), audit)
    print(
        json.dumps(
            {
