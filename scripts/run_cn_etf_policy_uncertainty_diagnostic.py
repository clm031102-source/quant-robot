"""Preflight or execute the exact, separately admitted monthly gross diagnostic."""
...
from quant_robot.research.monthly_diagnostic_registration import DIRECTORY, resolve_path, runtime_environment
...
def run(root, *, execute=False):
    ...
    if registration.get('source_origin') == 'retained_research_sources':
        ...
        if changed:
            raise ValueError('commit the reviewed code and admission before a real-input run')
    ...
    def gate_supplier():
        ...
        write_quant_pm_startup_gate(root / DIRECTORY / 'pm_gate', pack)
        return pack
    args = dict(root=root, registration_path=registration_path, scheduler=scheduler,
        gate_supplier=gate_supplier, environment=runtime_environment())
    if not execute:
        prepared = preflight_registration(**args)
        return {'status': 'ready_unconsumed', 'registration_id': prepared.registration['registration_id'],
            'input_files': len(prepared.snapshots), 'factor_generated': False, 'prices_decoded': False,
            'execution_claim_recorded': False}
    result = execute_registration(**args)
    return {'status': 'completed', 'decision': result['diagnostic']['decision'],
        'source_origin': result['source_origin'], 'intervals': result['diagnostic']['interval_count'],
