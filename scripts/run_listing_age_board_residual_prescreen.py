...
from quant_robot.factors.listing_age_board import LISTING_AGE_BOARD_FACTOR_NAMES  # noqa: E402
from quant_robot.ops.capacity_safe_price_volume_prescreen import (  # noqa: E402
    DEFAULT_ANALYSIS_END_DATE,
    DEFAULT_ANALYSIS_START_DATE,
)
from quant_robot.ops.listing_age_board_residual_prescreen import (  # noqa: E402
    build_listing_age_board_residual_prescreen,
    build_listing_age_board_sharded_residual_prescreen,
...
DEFAULT_OUTPUT_DIR = Path("data/reports/listing_age_board_residual_prescreen_round259_20260626")


def run_listing_age_board_residual_prescreen_cli(
    *,
    ...
    if factor_frame is not None and labels is not None:
        ...
        result = build_listing_age_board_sharded_residual_prescreen(
        ...
        result = build_listing_age_board_residual_prescreen(
            bars_roots=tuple(Path(path) for path in bars_roots),
            stock_basic=Path(stock_basic) if isinstance(stock_basic, (str, Path)) else stock_basic,
            candidate_factor_names=candidate_factor_names,
            analysis_start_date=analysis_start_date,
    ...
    write_listing_age_board_residual_prescreen(output_dir, result)
    return result


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Round259 listing-age/board structural residual IC and reference de-duplication prescreen."
    )
    ...
    result = run_listing_age_board_residual_prescreen_cli(
        ...
        candidate_factor_names=tuple(args.candidate_factor_names or LISTING_AGE_BOARD_FACTOR_NAMES),
