...
from quant_robot.ops.profitability_event_revision_controlled_ic_neutral_prescreen import (  # noqa: E402
    build_profitability_event_revision_controlled_ic_neutral_prescreen,
    write_profitability_event_revision_controlled_ic_neutral_prescreen,
)
...
DEFAULT_OUTPUT_DIR = Path("data/reports/profitability_event_revision_controlled_ic_neutral_prescreen_round153_20260623")


def run_profitability_event_revision_controlled_ic_neutral_prescreen_cli(
    *,
    ...
    write_profitability_event_revision_controlled_ic_neutral_prescreen(output_dir, result)
    if not allow_not_ready and not result["summary"].get("passes", False):
        blockers = ", ".join(result["summary"].get("blockers", []) or [])
        raise RuntimeError(f"PIT profitability event-revision controlled IC prescreen is not ready: {blockers}")
    return result


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Round153 PIT profitability event/revision controlled IC, neutralization, and Round96 de-dup prescreen."
    )
