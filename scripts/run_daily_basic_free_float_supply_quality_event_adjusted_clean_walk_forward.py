...
from quant_robot.ops.daily_basic_free_float_supply_quality_event_adjusted_clean_walk_forward import (  # noqa: E402
    DEFAULT_MIN_ACCEPTED_FOLDS,
    DEFAULT_MIN_TEST_TOTAL_RETURN,
    DEFAULT_MIN_TEST_TRADES,
    DEFAULT_ROLLING_STEP_DAYS,
    DEFAULT_ROLLING_TEST_DAYS,
    DEFAULT_ROLLING_TRAIN_DAYS,
    DEFAULT_WALK_FORWARD_GUARD_MODES,
    build_daily_basic_free_float_supply_quality_event_adjusted_clean_walk_forward,
    write_daily_basic_free_float_supply_quality_event_adjusted_clean_walk_forward,
)
...
DEFAULT_OUTPUT_DIR = Path(
    "data/reports/daily_basic_free_float_supply_quality_event_adjusted_clean_walk_forward_round141_20260622"
)


def run_daily_basic_free_float_supply_quality_event_adjusted_clean_walk_forward_cli(
    *,
    ...
    result = build_daily_basic_free_float_supply_quality_event_adjusted_clean_walk_forward(
    ...
    write_daily_basic_free_float_supply_quality_event_adjusted_clean_walk_forward(output_dir, result)
    return result
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run Round141 clean walk-forward validation for event-adjusted daily-basic free-float supply quality."
        )
    )
    ...
    parser.add_argument("--guard-modes", default=",".join(DEFAULT_WALK_FORWARD_GUARD_MODES))
    ...
    parser.add_argument("--rolling-train-days", type=int, default=DEFAULT_ROLLING_TRAIN_DAYS)
    parser.add_argument("--rolling-test-days", type=int, default=DEFAULT_ROLLING_TEST_DAYS)
    parser.add_argument("--rolling-step-days", type=int, default=DEFAULT_ROLLING_STEP_DAYS)
    parser.add_argument("--min-test-trades", type=int, default=DEFAULT_MIN_TEST_TRADES)
    parser.add_argument("--min-accepted-folds", type=int, default=DEFAULT_MIN_ACCEPTED_FOLDS)
    parser.add_argument("--min-test-total-return", type=float, default=DEFAULT_MIN_TEST_TOTAL_RETURN)
    ...
    result = run_daily_basic_free_float_supply_quality_event_adjusted_clean_walk_forward_cli(
        ...
        min_test_total_return=args.min_test_total_return,
    )
    print(
        json.dumps(
            {
