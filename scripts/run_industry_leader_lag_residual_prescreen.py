...
from quant_robot.factors.industry_leader_lag import INDUSTRY_LEADER_LAG_FACTOR_NAMES  # noqa: E402
from quant_robot.ops.capacity_safe_price_volume_prescreen import (  # noqa: E402
    DEFAULT_ANALYSIS_END_DATE,
    DEFAULT_ANALYSIS_START_DATE,
)
from quant_robot.ops.industry_leader_lag_residual_prescreen import (  # noqa: E402
    build_industry_leader_lag_residual_prescreen,
...
DEFAULT_OUTPUT_DIR = Path("data/reports/industry_leader_lag_residual_prescreen_round220_20260624")


def run_industry_leader_lag_residual_prescreen_cli(
    *,
    ...
    if factor_frame is not None and labels is not None:
        ...
        result = build_industry_leader_lag_residual_prescreen(
            bars_roots=tuple(Path(path) for path in bars_roots),
            stock_basic=Path(stock_basic) if isinstance(stock_basic, (str, Path)) else stock_basic,
            candidate_factor_names=candidate_factor_names,
            analysis_start_date=analysis_start_date,
    ...
    write_industry_leader_lag_residual_prescreen(output_dir, result)
    return result


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Round220 industry leader-lag residual IC and reference de-duplication prescreen."
    )
    ...
    result = run_industry_leader_lag_residual_prescreen_cli(
        ...
        candidate_factor_names=tuple(args.candidate_factor_names or INDUSTRY_LEADER_LAG_FACTOR_NAMES),
