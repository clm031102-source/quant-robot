...
from quant_robot.ops.public_reference_multi_family_prescreen import (  # noqa: E402
    ...
    write_public_reference_multi_family_prescreen,
)


DEFAULT_OUTPUT_DIR = Path("data/reports/public_reference_multi_family_prescreen_round128_20260622")


def run_public_reference_multi_family_prescreen_cli(
    *,
    ...
    write_public_reference_multi_family_prescreen(output_dir, result)
    return result


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run CN stock public-reference multi-family IC/quantile/turnover prescreen."
    )
    parser.add_argument("--bars-root", action="append", required=True)
    parser.add_argument("--factor-input-root", required=True)
    parser.add_argument("--moneyflow-input-root", required=True)
