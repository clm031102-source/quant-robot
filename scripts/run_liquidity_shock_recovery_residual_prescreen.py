...
from quant_robot.factors.liquidity_shock_recovery import (  # noqa: E402
...
from quant_robot.ops.liquidity_shock_recovery_residual_prescreen import (  # noqa: E402
    build_liquidity_shock_recovery_residual_prescreen,
    build_liquidity_shock_recovery_sharded_residual_prescreen,
...
DEFAULT_OUTPUT_DIR = Path("data/reports/liquidity_shock_recovery_residual_prescreen_round230_20260624")


def run_liquidity_shock_recovery_residual_prescreen_cli(
    *,
    ...
    if factor_frame is not None and labels is not None:
        ...
        result = build_liquidity_shock_recovery_sharded_residual_prescreen(
        ...
        result = build_liquidity_shock_recovery_residual_prescreen(
            bars_roots=tuple(Path(path) for path in bars_roots),
            stock_basic=Path(stock_basic) if isinstance(stock_basic, (str, Path)) else stock_basic,
            candidate_factor_names=candidate_factor_names,
            analysis_start_date=analysis_start_date,
    ...
    write_liquidity_shock_recovery_residual_prescreen(output_dir, result)
    return result


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Round230 liquidity shock recovery long-cycle residual prescreen."
    )
    ...
    parser.add_argument(
        "--reference-mode",
        choices=("defer_until_residual_lead", "full"),
        default="defer_until_residual_lead",
    )
    ...
    result = run_liquidity_shock_recovery_residual_prescreen_cli(
        ...
        candidate_factor_names=tuple(args.candidate_factor_names or LIQUIDITY_SHOCK_RECOVERY_FACTOR_NAMES),
