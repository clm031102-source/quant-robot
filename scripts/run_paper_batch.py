...
def load_paper_batch_config(path: str | Path) -> PaperBatchConfig:
    ...
    return PaperBatchConfig(
        walk_forward_leaderboard=Path(data["walk_forward_leaderboard"]),
...
def run_paper_batch(
    ...
    rows = _candidate_rows(config.walk_forward_leaderboard, config.max_candidates)
...
def _candidate_rows(path: Path, max_candidates: int | None) -> list[dict[str, Any]]:
...
def _run_candidate(row: dict[str, Any], config: PaperBatchConfig) -> dict[str, Any]:
    ...
    if skip_reason:
        return _candidate_summary(row, status="skipped", error=str(skip_reason))
...
def _config_dict(config: PaperBatchConfig) -> dict[str, Any]:
    return {
        "walk_forward_leaderboard": str(config.walk_forward_leaderboard),
        "source": config.source,
