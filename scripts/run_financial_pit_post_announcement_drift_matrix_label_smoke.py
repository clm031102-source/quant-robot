...
from quant_robot.ops.financial_pit_post_announcement_drift_matrix_label_smoke import (  # noqa: E402
...
DEFAULT_OUTPUT_DIR = Path("data/reports/financial_pit_post_announcement_drift_matrix_label_smoke_round222_20260624")


def run_financial_pit_post_announcement_drift_matrix_label_smoke_cli(
    *,
    ...
    if not allow_not_ready and not result["summary"].get("passes", False):
        ...
        raise RuntimeError(f"Financial PIT post-announcement drift matrix label smoke is not ready: {blockers}")
    return result


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Round222 financial PIT post-announcement drift factor-matrix and label-alignment smoke."
    )
