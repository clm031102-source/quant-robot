...
DEFAULT_OUTPUT_DIR = Path("data/reports/shortlist_public_factor_entry_tilt")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Project public-factor selected-entry exposure tilts onto a frozen official event-return template."
    )
    ...
    audit = build_public_factor_entry_tilt_audit(
        ...
        candidates=tuple(parse_public_factor_tilt_spec(value) for value in args.candidate),
    ...
    write_public_factor_entry_tilt_audit(Path(args.output_dir), audit)
    print(
        json.dumps(
            {
