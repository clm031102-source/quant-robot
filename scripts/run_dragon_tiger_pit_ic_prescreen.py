...
from quant_robot.ops.dragon_tiger_pit_ic_prescreen import (  # noqa: E402
...
DEFAULT_CANDIDATE_PLAN = Path("configs/factor_mining_candidate_plan_round232_dragon_tiger_attention_reversal_20260624.json")
DEFAULT_OUTPUT_DIR = Path("data/reports/round232_dragon_tiger_pit_ic_prescreen_20260625")


def run_dragon_tiger_pit_ic_prescreen_cli(
    *,
    ...
    candidate_plan_json: str | Path | None = DEFAULT_CANDIDATE_PLAN,
    ...
    result = build_dragon_tiger_pit_ic_prescreen(
        ...
        candidate_plan_json=Path(candidate_plan_json) if candidate_plan_json else None,
    ...
    write_dragon_tiger_pit_ic_prescreen(output_dir, result)
    if not allow_not_ready and result["summary"].get("factor_rows", 0) == 0:
        raise RuntimeError("Dragon-Tiger PIT IC prescreen produced no factor rows")
    return result


def main() -> None:
    parser = argparse.ArgumentParser(description="Run Round232 Dragon-Tiger point-in-time event IC prescreen.")
    ...
    result = run_dragon_tiger_pit_ic_prescreen_cli(
        ...
        candidate_plan_json=Path(args.candidate_plan_json) if args.candidate_plan_json else None,
