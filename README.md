...
- Integration candidate: the 141 commits from `origin/codex/factor-review-etf-monetization-20260911` at `0b8984c` are included in this review branch with their accounting, execution, source-receipt and research-closeout evidence. Mainline integration remains subject to validation.
...
The residual-regime config enables `precompute_factor_matrix`, `reuse_research_inputs`, minimal case evidence, and strict fingerprinted resume. Rolling validation lazily computes one causal factor matrix for the frozen 2015-2025 input window, slices it by fold, and shares each slice across that fold's train/test grids. The requested combo dependencies are pruned before factor construction, while cases with the same factor, regime, and signal rules reuse labels, IC/group inputs, and benchmark preparation. Every TopN/cost/capacity portfolio is still backtested separately. Completed grids are reused only when code/config/data/environment fingerprints match. Train grids retain manifests and leaderboards without case files; test cases atomically retain only `regime_curve.csv` and final `metrics.json`, which are the evidence required for regime coverage and safe resume.
